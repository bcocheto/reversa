...
Salve as respostas em `.agentforge/state.json` nos campos `user_name`, `chat_language`, `doc_language` e `project`.
...
>
> Trabalharei em etapas, salvando o progresso a cada fase. Se a sessão for interrompida, basta digitar `agentforge` novamente para continuar de onde paramos."

## 5. Plano de exploração

Verifique se `.agentforge/plan.md` já existe:
...
1. Analise rapidamente a estrutura de pastas raiz (exclua: `node_modules`, `.git`, `.agentforge`, `_agentforge`, `dist`, `build`, `coverage`, `__pycache__`)
2. Identifique os módulos e componentes principais
3. Crie `.agentforge/plan.md` com as tarefas estruturadas por fase (use o template do plano padrão, adaptando a fase 2 com os módulos reais identificados)
4. Apresente o plano e pergunte: "O plano está aprovado ou quer ajustar algo?"

## 6. Atualização do estado

Após aprovação do plano, atualize `.agentforge/state.json`:
...
Após confirmação, ative o skill `agentforge-scout`.
