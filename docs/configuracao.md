...
agentforge stores all its configuration and analysis state inside the `.agentforge/` folder at the project root. You can open and edit the files whenever you want.

---

## The `.agentforge/` folder structure
...
├── version             ← installed agentforge version
├── context/
...
You can change the output `folder` if you prefer a different name than `_agentforge_sdd`.
...
agentforge generates this file in the first session, after talking with you about the project. It lists the analysis tasks in order.

You can edit it directly: reorder tasks, remove modules you don't want to analyze, add notes. agentforge will respect whatever is here when it resumes.
...
| `file` | The Reviewer generates a `_agentforge_sdd/questions.md` file with all questions. You fill it in and notify when done. |
...
Defines the volume of artifacts each agent generates during the analysis. **Not configured at install time:** agentforge asks at the start of the first analysis session, after the Scout maps the project, so you can decide with real information in hand.
...
The choice is saved in `.agentforge/state.json` under the `doc_level` field. You can edit it manually at any time to adjust the level mid-analysis.
