...
import { renderCommandsReferenceDocument } from '../commands/registry.js';
import { renderPlanFromPhases } from '../commands/phase-engine.js';
...
export const CANONICAL_AGENT_TEMPLATES = Object.freeze({
  ...
  'context-curator': 'agentforge/agents/context-curator.yaml',
...
export const CANONICAL_FLOW_TEMPLATES = Object.freeze({
  ...
  review: 'agentforge/flows/review.yaml',
  'context-curation': 'agentforge/flows/context-curation.yaml',
...
export const CANONICAL_FLOW_DEPENDENCIES = Object.freeze({
  ...
  review: ['orchestrator', 'reviewer', 'qa', 'security'],
  'context-curation': ['orchestrator', 'context-curator', 'reviewer'],
...
const MANAGED_BOOTLOADER_ENTRY_TEMPLATES = Object.freeze({
  ...
  'GEMINI.md': {
    activationText: 'Quando o usuário digitar `agentforge`, ative o AgentForge na Gemini CLI.',
  },
});
...
export const MINIMUM_HUMAN_READABLE_STRUCTURE_TEMPLATES = Object.freeze([
  ...
  { template: 'agentforge/harness/context-map.yaml', dest: 'harness/context-map.yaml' },
  ...
  { template: 'agentforge/ai/README.md', dest: 'ai/README.md' },
  { template: 'agentforge/workflow/phases.yaml', dest: 'workflow/phases.yaml' },
  { template: 'agentforge/workflow/history.jsonl', dest: 'workflow/history.jsonl' },
...
export const HUMAN_READABLE_STRUCTURE_TEMPLATES = Object.freeze([
  ...
  { template: 'agentforge/harness/engine-map.yaml', dest: 'harness/engine-map.yaml' },
  { template: 'agentforge/ai/README.md', dest: 'ai/README.md' },
  { template: 'agentforge/ai/playbooks/discovery.md', dest: 'ai/playbooks/discovery.md' },
  { template: 'agentforge/ai/playbooks/agent-design.md', dest: 'ai/playbooks/agent-design.md' },
  { template: 'agentforge/ai/playbooks/flow-design.md', dest: 'ai/playbooks/flow-design.md' },
  { template: 'agentforge/ai/playbooks/policies.md', dest: 'ai/playbooks/policies.md' },
  { template: 'agentforge/ai/playbooks/export.md', dest: 'ai/playbooks/export.md' },
  { template: 'agentforge/ai/playbooks/review.md', dest: 'ai/playbooks/review.md' },
  { template: 'agentforge/ai/playbooks/task-execution.md', dest: 'ai/playbooks/task-execution.md' },
  { template: 'agentforge/ai/engines/codex.md', dest: 'ai/engines/codex.md' },
  { template: 'agentforge/ai/engines/claude.md', dest: 'ai/engines/claude.md' },
  { template: 'agentforge/ai/engines/gemini.md', dest: 'ai/engines/gemini.md' },
  { template: 'agentforge/ai/engines/cursor.md', dest: 'ai/engines/cursor.md' },
  { template: 'agentforge/ai/engines/copilot.md', dest: 'ai/engines/copilot.md' },
  { template: 'agentforge/workflow/phases.yaml', dest: 'workflow/phases.yaml' },
  { template: 'agentforge/workflow/history.jsonl', dest: 'workflow/history.jsonl' },
  ...
  { template: 'agentforge/context/unclassified.md', dest: 'context/unclassified.md' },
  ...
  { template: 'agentforge/references/tools.md', dest: 'references/tools.md' },
  ...
  { template: 'agentforge/flows/context-curation.md', dest: 'flows/context-curation.md' },
  { template: 'agentforge/flows/release.md', dest: 'flows/release.md' },
...
function canonicalFlows(values = []) {
  return unique(values).filter((flowId) => Object.hasOwn(CANONICAL_FLOW_TEMPLATES, flowId));
}
...
export class Writer {
  ...
  _writeTemplate(templateRelPath, destPath, { force = false } = {}) {
    ...
    const content = templateRelPath === 'agentforge/references/commands.md'
      ? renderCommandsReferenceDocument()
  ...
  installCanonicalStructure(internalDirName, { agents = [], flows = [], scopeContent = null, force = false } = {}) {
    ...
    for (const subagentId of DEFAULT_GENERATED_SUBAGENT_IDS) {
      this._writeTemplate(
  ...
  async installEntryFile(engine, { force = false, deferExistingEntrypoints = false } = {}) {
    ...
    const content = bootloaderTemplate
      ? renderManagedEntrypoint({ ...bootloaderTemplate, entryFile: engine.entryFile })
      : readFileSync(templatePath, 'utf8');
    ...
    if (!existsSync(destPath)) {
      ...
      writeFileSync(destPath, content, 'utf8');
      this._register(destPath);
      return { status: 'created', path: destPath };
    }

    if (deferExistingEntrypoints) {
      return { status: 'deferred', path: destPath };
    }
    ...
    if (strategy === 'merge') {
      ...
      return { status: 'merged', path: destPath };
    }
    // 'skip' → não faz nada
  ...
  createProductDir(answers, version) {
    ...
    const projectName = String(answers.project_name ?? answers.project ?? '').trim();
    const projectType = String(answers.project_type ?? 'SaaS/Web App').trim() || 'SaaS/Web App';
    const setupMode = normalizeSetupMode(answers.setup_mode);
    const stack = String(answers.stack ?? '').trim() || 'Não detectado ainda';
    const objective = String(answers.objective ?? '').trim() || inferObjective(setupMode);
    const gitStrategy = String(answers.git_strategy ?? 'commit').trim() || 'commit';
    const internalAgents = unique(answers.internal_agents ?? AGENT_SKILL_IDS);
    const initialAgents = unique(answers.initial_agents ?? answers.generated_agents ?? inferInitialAgents({}, setupMode));
    const initialFlows = unique(answers.initial_flows ?? answers.flows ?? inferInitialFlows({}, setupMode));
    const canonicalInitialFlows = canonicalFlows(initialFlows);
    const generatedSubagents = [...DEFAULT_GENERATED_SUBAGENT_IDS];
    ...
    const selectedAgents = unique([
      ...initialAgents,
      ...canonicalInitialFlows.flatMap(flowId => CANONICAL_FLOW_DEPENDENCIES[flowId] ?? []),
    ]);
    ...
    const scopeContent = render(scopeTemplate, {
      ...
      INITIAL_FLOWS: formatDisplayList(canonicalInitialFlows, FLOW_LABELS),
    ...
    state.initial_flows = canonicalInitialFlows;
    state.flows = canonicalInitialFlows;
    ...
    const initialFlowsList = formatTomlArray(canonicalInitialFlows);
    ...
    const flowsList = formatTomlArray(canonicalInitialFlows);
    ...
    const stackStatus = stack === 'Não detectado ainda' ? 'A preencher' : 'Detectado';
    const bootstrapReport = [
      ...
      `- Stack: ${stack}`,
      `- Status da stack: ${stackStatus}`,
      `- Objetivo: ${objective}`,
      `- Agentes iniciais: ${formatDisplayList(initialAgents, AGENT_LABELS)}`,
      `- Fluxos iniciais: ${formatDisplayList(initialFlows, FLOW_LABELS)}`,
      `- Engines: ${formatDisplayList(answers.engines ?? [])}`,
      '',
    ].join('\n');
    this._writeNew(join(internalDir, 'reports', 'bootstrap.md'), bootstrapReport);
    ...
    this.installCanonicalStructure(internalDirName, {
      ...
      flows: canonicalInitialFlows,
      scopeContent,
    });

    // plan.md
    const plan = renderPlanFromPhases(this.projectRoot, state);
  ...
  saveCreatedFiles() {
    ...
    const planPath = join(this.projectRoot, resolveInternalDir(this.projectRoot), 'plan.md');
