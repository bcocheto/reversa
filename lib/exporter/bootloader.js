...
export function buildManagedBootloaderLines({
  ...
  if (workflowComplete) {
    const managedLines = [
      activationText,
      'O ciclo AgentForge já está concluído neste projeto.',
      'Task packs e fluxos operacionais estão prontos para uso.',
      'Peça um objetivo de tarefa real e siga a política local antes de editar arquivos.',
      'Se o binário local não existir, use `npx @bcocheto/agentforge <command>`.',
      ...
      'Consulte `.agentforge/harness/router.md` e `.agentforge/harness/context-index.yaml` apenas para orientação estrutural.',
    ];
  ...
  const managedLines = [
    ...
    'Use `agentforge next` para determinar a próxima fase e `agentforge advance` para avançar.',
    'Se o binário local não existir, use `npx @bcocheto/agentforge advance`.',
    'Nunca edite `state.json` ou `plan.md` manualmente.',
