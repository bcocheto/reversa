...
function buildExportTargets(context, rootPrefix = '') {
  const targets = [];
...
function renderExportReport(context, output, { takeoverEntrypoints = false, packageMode = false } = {}) {
  const lines = [];
...
export async function compileAgentForge(projectRoot, { force = false, takeoverEntrypoints = false } = {}) {
  ...
  writeFileSync(reportPath, renderExportReport(context, output, { takeoverEntrypoints }), 'utf8');
