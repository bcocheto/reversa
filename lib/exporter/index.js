...
function buildExportTargets(context, rootPrefix = '', { includeExistingEntrypoints = false } = {}) {
  ...
  if (engineSet.has('gemini') || engineSet.has('gemini-cli')) {
    const bootloader = {
      entryFile: 'GEMINI.md',
      activationText: 'Abra Gemini CLI e digite `agentforge`.',
    };
    addTarget({
      kind: 'entrypoint',
      path: withPrefix('GEMINI.md'),
      engine: bootloader,
      content: renderManagedEntrypoint(bootloader, state),
      managedLines: buildManagedBootloaderLines({
        activationText: bootloader.activationText,
...
export async function compileAgentForge(projectRoot, {
  force = false,
