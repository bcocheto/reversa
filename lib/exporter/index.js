...
async function writeTarget(projectRoot, target, manifest, force, takeoverEntrypoints, output) {
...
function renderCompileReport(context, output, { takeoverEntrypoints = false } = {}) {
  const lines = [];
...
export async function compileAgentForge(projectRoot, { force = false, takeoverEntrypoints = false } = {}) {
  ...
  writeFileSync(reportPath, renderCompileReport(context, output, { takeoverEntrypoints }), 'utf8');
