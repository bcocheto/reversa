...
function renderBootloaderEntry({ activationText, activationHint, frontmatterLines = [] }) {
  const managedLines = [
    activationText,
  ...
  return {
    managedLines,
    content: renderBootloaderDocument({
...
function buildExportTargets(context) {
  ...
  if (engineSet.has('codex')) {
    const bootloader = renderBootloaderEntry({
      activationText: 'Quando o usuário digitar `agentforge`, ative o orquestrador AgentForge.',
    });
    targets.push({
      kind: 'bootloader',
      path: 'AGENTS.md',
      content: bootloader.content,
      managedLines: bootloader.managedLines,
    });
  }
...
function renderCompileReport(context, output) {
  ...
  if (output.warnings.length > 0) {
    lines.push('## Warnings');
    lines.push('');
    lines.push(output.warnings.map((entry) => `- ${entry}`).join('\n'));
    lines.push('');
  }
...
export function renderManagedBootloaderDocument(options) {
  return renderBootloaderDocument(options);
}
