import { renderAgenticBlueprintSchema } from './agentic-blueprint-schema.js';
...
export function renderAgenticBlueprintRequest(bundle) {
  const lines = [];
  lines.push('# Agentic Blueprint Request');
  lines.push('');
  lines.push('## Goal');
  lines.push('');
  lines.push('Leia o evidence bundle completo antes de decidir.');
  lines.push('Julgue semanticamente o projeto e a arquitetura agentic, não apenas por palavras-chave, heurísticas ou presença superficial de arquivos.');
  lines.push('Decida a arquitetura agentic completa do projeto com base apenas na evidência do bundle.');
  lines.push('Decida semanticamente quais agentes são necessários, quais skills cada agente precisa, quais context_documents cada agente usará, e quais flows, policies e entrypoints fazem sentido.');
  lines.push('Preencha `.agentforge/ai/outbox/agentic-blueprint.yaml`.');
  lines.push('Os sinais heurísticos do CLI não são recomendação final.');
  lines.push('Não copie heurísticas do CLI como decisão final.');
  lines.push('');
  lines.push('## Constraints');
  lines.push('');
  lines.push('- Não crie agente sem evidência.');
  lines.push('- Não crie o agente genérico "reviewer" sem papel específico.');
  lines.push('- Não materialize arquivos manualmente.');
  lines.push('- Não altere state/manifest.');
  lines.push('- Justifique tudo com source_evidence.');
  lines.push('- Evite papéis genéricos, duplicados ou sobrepostos.');
  lines.push('- Se um item não tiver evidência suficiente, omita-o.');
  lines.push('');
  lines.push('## Evidence bundle summary');
  lines.push('');
  lines.push(...formatBundleSummary(bundle));
  lines.push('');
  lines.push('## Evidence inventory');
  lines.push('');
  lines.push(...formatEvidenceInventory(bundle));
  lines.push('');
  lines.push('## Output schema');
  lines.push('');
  lines.push('Retorne somente YAML válido usando o schema `agentic-blueprint`.');
  lines.push('');
  lines.push(renderAgenticBlueprintSchema().trimEnd());
  lines.push('');
  lines.push('## Output rules');
  lines.push('');
  lines.push('- Responda somente com YAML válido.');
  lines.push('- O YAML deve preencher os campos agents, skills, context_documents, flows, policies, routing, entrypoints, exports e migration_plan quando houver evidência.');
  lines.push('- Cada agent, skill, context document, flow, policy, routing rule, entrypoint, export e migration step deve carregar source_evidence.');
  lines.push('- source_evidence deve usar paths reais vindos do evidence bundle e não pode inventar arquivos.');
  lines.push('- Use nomes e ids estáveis, específicos e não genéricos.');
  lines.push('- Não repita o mesmo papel em múltiplos agentes sem diferença semântica clara.');
  lines.push('- Não traduza sinais heurísticos do CLI em decisão final sem justificar com evidence bundle.');
  lines.push('');
  return `${lines.join('\n').trimEnd()}\n`;
}
