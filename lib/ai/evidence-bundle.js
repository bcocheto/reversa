...
export function buildAiEvidenceBundle(projectRoot, options = {}) {
  ...
  return {
    ...
    docsDetected: [
      ...(signals.readmeExists ? [{ path: 'README.md', content: signals.readmeText }] : []),
      ...(signals.docsFiles ?? []),
    ],
...
export function renderAiEvidenceBrief(bundle) {
  ...
  lines.push(renderSection('Docs Detected', renderEvidenceList(bundle.docsDetected?.map((doc) => ({
    path: doc.path,
    kind: 'doc',
    reason: 'Arquivo de documentação detectado.',
    snippet: snippetFromText(doc.content),
  })) ?? [])));
