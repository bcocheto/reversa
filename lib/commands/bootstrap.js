...
export function writeCoreContextFiles(projectRoot, state = null, parsedArgs = {}, { writer, manifest } = {}) {
...
function renderCommands(context) {
  ...
  lines.push('');
  const packageScriptEntries = context.commandEntries.filter((entry) => entry.source === 'package.json');
  lines.push(...formatCommandList(packageScriptEntries, 'Não detectado.'));
  lines.push('');
  ...
  lines.push('- `agentforge export`');
  lines.push('- `agentforge export-package`');
  lines.push('- `agentforge update`');
  lines.push('- `agentforge uninstall`');
  lines.push('');
  lines.push('## Imported commands');
  lines.push('');
  const importedCommands = context.commandEntries.filter((entry) => entry.source !== 'package.json');
  lines.push(...formatCommandList(importedCommands, 'Não detectado.'));
  const missing = [];
  if (packageScriptEntries.length === 0) missing.push('Scripts reais do package.json');
  if (importedCommands.length === 0) missing.push('Comandos importados de AGENTS/CLAUDE/docs');
  if (missing.length > 0) {
    lines.push('');
    lines.push('## A preencher');
    lines.push('');
