  ...
  try {
    const answers = await inquirer.prompt([
      ...
      },
      {
        ...
        validate: (value) => (normalizeString(value) ? true : 'A missão é obrigatória.'),
      },
      {
        type: 'input',
        name: 'responsibilities',
        message: 'Responsabilidades principais (separe por vírgula ou nova linha):',
        validate: (value) => (splitListInput(value).length > 0 ? true : 'Informe ao menos uma responsabilidade.'),
      },
      {
        type: 'input',
        name: 'can_change',
        message: 'Arquivos/pastas que pode alterar (separe por vírgula ou nova linha):',
        validate: (value) => (splitListInput(value).length > 0 ? true : 'Informe ao menos um caminho permitido.'),
      },
      {
        type: 'input',
        name: 'cannot_change',
        message: 'Arquivos/pastas que não pode alterar (separe por vírgula ou nova linha):',
        validate: (value) => (splitListInput(value).length > 0 ? true : 'Informe ao menos um caminho proibido.'),
      },
      {
        type: 'input',
        name: 'human_approval',
        message: 'Situações que exigem aprovação humana (separe por vírgula ou nova linha):',
        default: '',
      },
      {
        type: 'input',
        name: 'activation_commands',
        message: 'Comandos de ativação opcionais (separe por vírgula ou nova linha):',
        default: '',
      },
      {
        type: 'confirm',
