...
export default async function addAgent(args) {
  ...
  const installedAgents = new Set(state.internal_agents ?? state.agents ?? []);
  ...
  try {
    ...
    console.log(chalk.red('  Could not read the internal agents folder.\n'));
    return;
  }

  if (availableAgents.length === 0) {
    console.log(chalk.hex('#ffa203')('  All available internal agents are already installed.\n'));
    return;
  }
  ...
  const { selected } = await inquirer.prompt([{
    ...
    type: 'checkbox',
    name: 'selected',
    message: 'Select internal agents to add:',
    choices,
  ...
  s.internal_agents = [...new Set([...(s.internal_agents ?? s.agents ?? []), ...selected])];
  delete s.agents;
  ...
  console.log(chalk.bold(`\n  ${selected.length} internal agent(s) added successfully.\n`));
}
