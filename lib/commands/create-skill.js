...
const SUGGESTION_DIR = join(PRODUCT.internalDir, 'skill-suggestions');
...
function renderSkillDocument(suggestion) {
  ...
  lines.push('');
  lines.push(`- Sugestão de origem: \`.agentforge/skill-suggestions/${suggestion.id}.yaml\``);
...
function updateContextIndex(projectRoot, suggestion) {
...
function renderHelp(chalk) {
  ...
  console.log('  Cria uma skill real a partir de uma sugestão em `.agentforge/skill-suggestions/`.');
...
export function createProjectSkill(projectRoot, skillId, { force = false } = {}) {
  ...
  const suggestionPath = join(projectRoot, SUGGESTION_DIR, `${normalizedSkillId}.yaml`);
  if (!existsSync(suggestionPath)) {
    return {
      ok: false,
      errors: [
        `Sugestão ausente em \`${rel(projectRoot, suggestionPath)}\`.`,
        'Execute `agentforge suggest-skills` para gerar a sugestão antes de criar a skill.',
      ],
    };
  }
  ...
  if (suggestionId !== normalizedSkillId) {
    return {
      ...
      errors: [
        ...
        'Corrija a sugestão ou execute `agentforge suggest-skills` novamente.',
      ],
    };
  }

  const suggestionName = normalizeString(suggestion.name);
  const suggestionDescription = normalizeString(suggestion.description);
  const suggestionReason = normalizeString(suggestion.reason);
  ...
  const contextIndexUpdate = updateContextIndex(projectRoot, {
    id: suggestionId,
    description: suggestionDescription,
  });
  ...
  return {
    ...
    contextIndex: contextIndexUpdate.updated
      ? {
