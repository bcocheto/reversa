...
function buildAgentSuggestions(analysis) {
  ...
  if (patterns.some((pattern) => pattern.id === 'context-gap-signal')) {
    push({
      id: 'reviewer',
      title: 'Reviewer',
      description: 'Review the consolidated analysis and separate risks from recommendations.',
      reason: 'Context gaps and reportable risks were detected.',
      confidence: 'medium',
...
function buildFlowSuggestions(analysis) {
  ...
  if (analysis.localPatterns.some((pattern) => pattern.id === 'command-driven-automation' || pattern.id === 'context-gap-signal')) {
    push({
      id: 'review',
      title: 'Review',
      description: 'Flow for reviewing changes, risks, and approval boundaries.',
      reason: 'The repo shows command-driven and context-sensitive review patterns.',
      confidence: 'medium',
...
function buildAnalysis(projectRoot, state, { ingestSummary } = {}) {
  ...
  const productSignals = detectProductSignals(signals, auditResult);
  ...
  const risks = detectRisks(signals, auditResult, extraCodeSurface);
  const localPatterns = detectLocalPatterns(signals, auditResult, extraCodeSurface);
  ...
  const analysis = {
    ...
    auditScore: typeof auditResult.analysis?.score === 'number' ? auditResult.analysis.score : 0,
  };
...
function persistAnalysis(projectRoot, state, analysisBundle, ingestSummary) {
