...
import { renderContextSynthesisRequest } from '../ai/request-renderer.js';
...
const CONTEXT_SYNTHESIS_REPORT_PATH = '.agentforge/reports/context-synthesis.md';
const AI_EVIDENCE_JSON_PATH = '.agentforge/ai/evidence/project-evidence.json';
const AI_EVIDENCE_BRIEF_PATH = '.agentforge/ai/evidence/project-brief.md';
const AI_SYNTHESIS_REQUEST_PATH = '.agentforge/ai/requests/synthesize-context.md';
...
function writeGeneratedArtifact(writer, projectRoot, manifest, relPath, content, { force = false } = {}) {
  if (!shouldWriteGeneratedArtifact(projectRoot, manifest, relPath, { force })) {
    return false;
  }
...
function shouldWriteGeneratedArtifact(projectRoot, manifest, relPath, { force = false } = {}) {
  ...
  return /(\bA preencher\b|<[^>]+>|\bTBD\b|\bNão detectado\b)/i.test(content);
}

function buildContextSynthesisReport({ analysisBundle, contextResult, evidenceBundle, requestPath, evidencePaths }) {
  const analysis = analysisBundle?.analysis ?? {};
  const signals = analysis?.signals ?? {};
  const expectedOutputs = [
    'Revisar os arquivos de contexto escritos pelo CLI.',
    'Confirmar ou corrigir campos marcados como `A confirmar` quando a evidência for fraca.',
    'Curar o contexto para refletir o projeto real, não a heurística local.',
    'Ajustar, criar ou remover documentos canônicos somente com base em evidências reais.',
  ];
  ...
  lines.push('## Status');
  lines.push('');
  lines.push('- O contexto escrito por `analyze --write-context` é uma síntese inicial, não a verdade final.');
  lines.push('- A IA ativa deve revisar e curar o contexto com base no request gerado abaixo.');
  lines.push('- Campos fracos ficam marcados como `A confirmar` para evitar excesso de certeza.');
  lines.push('');
  lines.push('## Artefatos');
  lines.push('');
  lines.push(`- Evidence bundle: \`${evidencePaths.json}\``);
  lines.push(`- Evidence brief: \`${evidencePaths.brief}\``);
  lines.push(`- IA request: \`${requestPath}\``);
  lines.push(`- Context docs written: ${contextResult?.writtenPaths?.length ?? 0}`);
  lines.push(`- Context docs skipped: ${contextResult?.skippedPaths?.length ?? 0}`);
  lines.push(`- Evidence items: ${evidenceBundle?.evidence?.length ?? 0}`);
  lines.push('');
  lines.push('## Expected AI outputs');
  lines.push('');
  ...
  lines.push('## Signals');
  lines.push('');
  lines.push(`- Project: ${signals.projectName || 'A confirmar'}`);
  lines.push(`- Objective: ${signals.objectiveText || 'A confirmar'}`);
  lines.push(`- Audience: ${signals.audienceText || 'A confirmar'}`);
  lines.push(`- Framework: ${analysis?.framework && analysis.framework !== 'Unknown' ? analysis.framework : 'A confirmar'}`);
  lines.push('');
  lines.push('## Evidence notes');
  lines.push('');
  lines.push('- O evidence bundle reutiliza sinais locais e pequenos trechos dos arquivos relevantes.');
  lines.push('- O request pede revisão semântica, prova real e sem sugestões genéricas.');
  lines.push('- Este relatório serve como guia de curadoria, não como decisão final.');
  lines.push('');

  return `${lines.join('\n').trimEnd()}\n`;
}

function updateContextSynthesisState(state, { analysisBundle, evidenceBundle, requestPaths, contextResult }) {
  return {
    ...(state ?? {}),
    last_context_synthesis_at: new Date().toISOString(),
    synthesized_context_files: contextResult?.writtenPaths ?? [],
    last_ai_context_request_at: new Date().toISOString(),
    ai_evidence: {
      project_name: evidenceBundle?.project?.name ?? null,
      project_type: evidenceBundle?.project?.type ?? null,
      framework: evidenceBundle?.stack?.framework ?? null,
      evidence_count: evidenceBundle?.evidence?.length ?? 0,
      files: requestPaths,
    },
    ai_context_request: {
      request_file: requestPaths.request,
      report_file: requestPaths.report,
      evidence_file: requestPaths.json,
      brief_file: requestPaths.brief,
      expected_outputs: [
        'Review and curate the synthesized context.',
        'Confirm weak fields or replace them with evidence-backed values.',
        'Split, merge, or rename context files if the repository evidence demands it.',
      ],
      source: 'analyze --write-context',
    },
    analysis_context_request: {
      analysis_at: new Date().toISOString(),
      report_file: requestPaths.report,
      evidence_file: requestPaths.json,
      request_file: requestPaths.request,
      evidence_items: evidenceBundle?.evidence?.length ?? 0,
      project: analysisBundle?.analysis?.signals?.projectName ?? null,
    },
  };
}
...
function persistContextPreparation(projectRoot, state, analysisBundle, { force = false } = {}) {
  const writer = new Writer(projectRoot);
  const manifest = loadManifest(projectRoot);
  const contextResult = applyCoreContextSynthesis(projectRoot, state, analysisBundle, { force });
  const evidenceBundle = buildAiEvidenceBundle(projectRoot, {
    state: contextResult.state,
    analysis: analysisBundle.analysis,
  });
  const requestText = renderContextSynthesisRequest(evidenceBundle);
  const requestReport = buildContextSynthesisReport({
    analysisBundle,
    contextResult,
    evidenceBundle,
    requestPath: AI_SYNTHESIS_REQUEST_PATH,
    evidencePaths: {
      json: AI_EVIDENCE_JSON_PATH,
      brief: AI_EVIDENCE_BRIEF_PATH,
      request: AI_SYNTHESIS_REQUEST_PATH,
      report: CONTEXT_SYNTHESIS_REPORT_PATH,
    },
  });

  const jsonText = `${JSON.stringify(evidenceBundle, null, 2)}\n`;
  const briefText = `${renderAiEvidenceBrief(evidenceBundle).trimEnd()}\n`;

  const writtenPaths = [...contextResult.writtenPaths];
  if (writeGeneratedArtifact(writer, projectRoot, manifest, AI_EVIDENCE_JSON_PATH, jsonText, { force })) {
    writtenPaths.push(AI_EVIDENCE_JSON_PATH);
  }
  if (writeGeneratedArtifact(writer, projectRoot, manifest, AI_EVIDENCE_BRIEF_PATH, briefText, { force })) {
    writtenPaths.push(AI_EVIDENCE_BRIEF_PATH);
  }
  if (writeGeneratedArtifact(writer, projectRoot, manifest, AI_SYNTHESIS_REQUEST_PATH, requestText, { force })) {
    writtenPaths.push(AI_SYNTHESIS_REQUEST_PATH);
  }
  if (writeGeneratedArtifact(writer, projectRoot, manifest, CONTEXT_SYNTHESIS_REPORT_PATH, requestReport, { force })) {
    writtenPaths.push(CONTEXT_SYNTHESIS_REPORT_PATH);
  }

  const nextState = updateContextSynthesisState(contextResult.state, {
    analysisBundle,
    evidenceBundle,
    requestPaths: {
      json: AI_EVIDENCE_JSON_PATH,
...
function renderAnalyzeHelp(chalk) {
  ...
  console.log('  Com --write-context, escreve a síntese inicial do contexto, um evidence bundle e um request para a IA ativa curar o contexto.\n');
}
...
export default async function analyze(args = []) {
  ...
  const contextResult = writeContext
    ? persistContextPreparation(projectRoot, persistResult.state, result.analysisBundle)
    : null;
  ...
  if (contextResult) {
    console.log(chalk.gray(`  Contexto inicial escrito: ${contextResult.contextResult.writtenPaths.length} arquivo(s)`));
    console.log(chalk.gray(`  Evidence bundle: ${AI_EVIDENCE_JSON_PATH}`));
    console.log(chalk.gray(`  IA request: ${AI_SYNTHESIS_REQUEST_PATH}`));
  }
