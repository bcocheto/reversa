...
import { ENTRYPOINT_MAX_LINES, countDocumentLines } from '../exporter/bootloader.js';
...
function validateManagedEntrypoints(projectRoot, manifest, errors, warnings) {
  for (const relPath of BOOTLOADER_ENTRY_FILES) {
    ...
    const hasBlock = hasBootloaderBlock(content);
    ...
    if ((hasManifestEntry || hasBlock) && lineCount > ENTRYPOINT_MAX_LINES) {
      pushError(errors, relPath, `Entrypoint gerenciado excede o limite de ${ENTRYPOINT_MAX_LINES} linhas (${lineCount}).`);
      continue;
    }
