...
import { validateContextMapFile } from './context-map.js';
import {
...
const ADOPTION_BLUEPRINT_REL_PATH = '.agentforge/ai/outbox/agentic-blueprint.yaml';
const ADOPTION_ACTIVE_STATUSES = new Set([
  'applying',
...
function normalizeAdoptionStateValue(value) {
  return normalizeString(value).toLowerCase();
}

function isAdoptionMaterializationActive(state) {
  const adoption = isPlainObject(state?.adoption) ? state.adoption : {};
  const values = [
    normalizeAdoptionStateValue(adoption.status),
    normalizeAdoptionStateValue(adoption.apply_status),
    normalizeAdoptionStateValue(state?.adoption_status),
  ].filter(Boolean);

  return values.some((value) => ADOPTION_ACTIVE_STATUSES.has(value));
}
...
function parseSkillFrontmatter(filePath, projectRoot, errors) {
  const relPath = rel(projectRoot, filePath);
  try {
    const raw = readFileSync(filePath, 'utf8');
    const match = raw.match(/^---\n([\s\S]*?)\n---\n([\s\S]*)$/);
    if (!match) {
      pushError(errors, relPath, 'SKILL.md precisa começar com frontmatter YAML delimitado por `---`.');
      return null;
    }

    const frontmatter = YAML.parse(match[1]);
    if (!frontmatter || typeof frontmatter !== 'object' || Array.isArray(frontmatter)) {
      pushError(errors, relPath, 'SKILL.md precisa conter um frontmatter YAML válido.');
      return null;
    }

    return { frontmatter, body: match[2] };
  } catch (error) {
    pushError(errors, relPath, `SKILL.md inválido: ${error.message}`);
    return null;
  }
}
...
function buildBlueprintValidationMessage(validationErrors = []) {
  if (!Array.isArray(validationErrors) || validationErrors.length === 0) {
    return 'Blueprint validado com sucesso.';
  }

  const parts = validationErrors.slice(0, 6).map((entry) => {
  ...
  return parts.length > 0
    ? `Blueprint inválido: ${parts.join('; ')}${validationErrors.length > parts.length ? '; ...' : ''}.`
    : 'Blueprint inválido.';
}

function validateAgenticAdoptionSurface(projectRoot, internalDir, state, errors) {
  const checks = [];
  const stateRef = rel(projectRoot, join(internalDir, 'state.json'));
  const stateBlueprintRelPath = normalizeBlueprintPath(state?.adoption?.blueprint_path);
  const outboxBlueprintAbsPath = join(projectRoot, ADOPTION_BLUEPRINT_REL_PATH);
  const blueprintRelPath = stateBlueprintRelPath || (existsSync(outboxBlueprintAbsPath) ? ADOPTION_BLUEPRINT_REL_PATH : '');
  const blueprintAbsPath = blueprintRelPath ? join(projectRoot, blueprintRelPath) : outboxBlueprintAbsPath;
  const blueprintExists = blueprintRelPath ? existsSync(blueprintAbsPath) : false;
  const adoptionActive = isAdoptionMaterializationActive(state);
  const hasBlueprintSource = Boolean(blueprintRelPath);

  if (!adoptionActive && !hasBlueprintSource) {
    return checks;
  }
  ...
  checks.push({
    id: 'blueprint_exists',
    passed: blueprintExists,
    message: blueprintExists
      ? `Blueprint encontrado em ${blueprintRelPath}.`
      : `Blueprint ausente em ${blueprintRelPath || ADOPTION_BLUEPRINT_REL_PATH}.`,
  });

  if (!blueprintExists) {
    if (adoptionActive) {
      pushError(errors, stateRef, `A adoção foi marcada como aplicada/em andamento sem ${blueprintRelPath}.`);
    }
    const missingMessage = 'Blueprint ausente; validação não executada.';
    for (const id of [
      'blueprint_valid',
      'agents_materialized',
      'skills_materialized',
      'context_materialized',
      'entrypoints_compiled',
      'context_index_matches_blueprint',
      'context_map_updated',
    ]) {
      checks.push({
        id,
  ...
  const blueprintDoc = parseYamlFile(blueprintAbsPath, errors, projectRoot);
  const evidenceBundle = buildAiEvidenceBundle(projectRoot, { state });
  const blueprintValidation = blueprintDoc
    ? validateAgenticBlueprint(blueprintDoc, evidenceBundle)
    : { valid: false, errors: [{ path: blueprintRelPath, message: 'Blueprint YAML inválido.' }], normalized: normalizeAgenticBlueprint({}) };
  const normalizedBlueprint = blueprintValidation.normalized?.blueprint ?? normalizeAgenticBlueprint(blueprintDoc ?? {}).blueprint;
  const blueprintValid = blueprintValidation.valid && (blueprintValidation.errors?.length ?? 0) === 0;

  checks.push({
    id: 'blueprint_valid',
    passed: blueprintValid,
    message: blueprintValid
      ? 'Blueprint validado com sucesso.'
      : buildBlueprintValidationMessage(blueprintValidation.errors),
  });

  if (!blueprintValid) {
    for (const error of blueprintValidation.errors ?? []) {
      pushError(errors, blueprintRelPath, `${normalizeString(error?.path) || blueprintRelPath}: ${normalizeString(error?.message) || 'Blueprint inválido.'}`);
    }
    for (const id of [
      'agents_materialized',
      'skills_materialized',
      ...
      checks.push({
        id,
        passed: false,
        message: 'Blueprint inválido; validação de materialização não executada.',
      });
    }
    return checks;
  }

  const blueprintAgentIds = new Set(normalizedBlueprint.agents.map((agent) => agent.id));
  const blueprintSkillIds = new Set(normalizedBlueprint.skills.map((skill) => skill.id));
  const blueprintEntrypointPaths = normalizedBlueprint.entrypoints.map((entrypoint) => normalizeBlueprintPath(entrypoint.path));

  const agentDir = join(internalDir, 'agents');
  const agentFiles = listYamlFiles(agentDir);
  const agentErrors = [];
  if (!existsSync(agentDir) || !statSync(agentDir).isDirectory()) {
    agentErrors.push('Diretório .agentforge/agents ausente.');
  }
  for (const agent of normalizedBlueprint.agents) {
    const agentFile = join(agentDir, `${agent.id}.yaml`);
    if (!existsSync(agentFile)) {
      agentErrors.push(`Falta ${rel(projectRoot, agentFile)}.`);
      continue;
    }
    const doc = parseYamlFile(agentFile, errors, projectRoot);
    if (!doc) {
      agentErrors.push(`YAML inválido em ${rel(projectRoot, agentFile)}.`);
      continue;
    }
    if (normalizeString(doc.id) !== agent.id) {
      agentErrors.push(`${rel(projectRoot, agentFile)} tem id diferente de ${agent.id}.`);
    }
    if (normalizeString(doc.generated_by) !== 'agentic-blueprint') {
      agentErrors.push(`${rel(projectRoot, agentFile)} não foi materializado pelo blueprint.`);
    }
    if (normalizeBlueprintPath(doc.blueprint_path) !== blueprintRelPath) {
      agentErrors.push(`${rel(projectRoot, agentFile)} não referencia ${blueprintRelPath}.`);
    }
  }
  for (const filePath of agentFiles) {
    const doc = parseYamlFile(filePath, errors, projectRoot);
    if (!doc) {
      agentErrors.push(`YAML inválido em ${rel(projectRoot, filePath)}.`);
      continue;
    }
    const id = normalizeString(doc.id) || fileId(filePath);
    const isBlueprintManaged = normalizeString(doc.generated_by) === 'agentic-blueprint'
      || normalizeBlueprintPath(doc.blueprint_path) === blueprintRelPath;
    if (!isBlueprintManaged) {
      continue;
    }
    if (!blueprintAgentIds.has(id)) {
      agentErrors.push(`${rel(projectRoot, filePath)} não está declarado no blueprint.`);
      continue;
    }
    if (normalizeString(doc.generated_by) !== 'agentic-blueprint') {
      agentErrors.push(`${rel(projectRoot, filePath)} não foi materializado pelo blueprint.`);
    }
    if (normalizeBlueprintPath(doc.blueprint_path) !== blueprintRelPath) {
      agentErrors.push(`${rel(projectRoot, filePath)} não aponta para ${blueprintRelPath}.`);
    }
  }
  checks.push({
    id: 'agents_materialized',
    passed: agentErrors.length === 0,
    message: agentErrors.length === 0
      ? 'Todos os agentes materializados correspondem ao blueprint.'
      : agentErrors.slice(0, 3).join(' '),
  });
  for (const message of agentErrors) {
    pushError(errors, rel(projectRoot, agentDir), message);
  }

  const skillDir = join(internalDir, 'skills');
  const skillFiles = listFilesRecursive(skillDir, (fullPath, name) => extname(name).toLowerCase() === '.md' && basename(name) === 'SKILL.md');
  const skillErrors = [];
  if (!existsSync(skillDir) || !statSync(skillDir).isDirectory()) {
    skillErrors.push('Diretório .agentforge/skills ausente.');
  }
  for (const skill of normalizedBlueprint.skills) {
    const skillPath = join(skillDir, skill.id, 'SKILL.md');
    if (!existsSync(skillPath)) {
      skillErrors.push(`Falta ${rel(projectRoot, skillPath)}.`);
      continue;
    }
    const parsed = parseSkillFrontmatter(skillPath, projectRoot, errors);
    if (!parsed) {
      skillErrors.push(`Frontmatter inválido em ${rel(projectRoot, skillPath)}.`);
      continue;
    }
    const metadata = isPlainObject(parsed.frontmatter.metadata) ? parsed.frontmatter.metadata : {};
    const ownerAgents = Array.isArray(metadata.owner_agents) ? metadata.owner_agents.map((entry) => normalizeString(entry)).filter(Boolean) : [];
    if (normalizeString(metadata.source) !== 'agentic-blueprint') {
      skillErrors.push(`${rel(projectRoot, skillPath)} não foi materializado pelo blueprint.`);
    }
    if (normalizeBlueprintPath(metadata.blueprint_path) !== blueprintRelPath) {
      skillErrors.push(`${rel(projectRoot, skillPath)} não aponta para ${blueprintRelPath}.`);
    }
    if (ownerAgents.length === 0) {
      skillErrors.push(`${rel(projectRoot, skillPath)} não declara agent owner.`);
    }
    for (const owner of ownerAgents) {
      if (!blueprintAgentIds.has(owner)) {
        skillErrors.push(`${rel(projectRoot, skillPath)} referencia owner inexistente: ${owner}.`);
      }
    }
  }
  for (const filePath of skillFiles) {
    const raw = readFileSync(filePath, 'utf8');
    if (!raw.includes('agentic-blueprint') && !raw.includes('blueprint_path')) {
      continue;
    }
    const parsed = parseSkillFrontmatter(filePath, projectRoot, errors);
    if (!parsed) {
      skillErrors.push(`Frontmatter inválido em ${rel(projectRoot, filePath)}.`);
      continue;
    }
    const metadata = isPlainObject(parsed.frontmatter.metadata) ? parsed.frontmatter.metadata : {};
    const source = normalizeString(metadata.source);
    const blueprintPath = normalizeBlueprintPath(metadata.blueprint_path);
    const suggestionId = normalizeString(metadata.skill_id) || normalizeString(metadata.suggestion_id) || basename(dirname(filePath));
    if (source === 'agentic-blueprint' || blueprintPath === blueprintRelPath) {
      if (!blueprintSkillIds.has(suggestionId)) {
        skillErrors.push(`${rel(projectRoot, filePath)} não está declarado no blueprint.`);
      }
      const ownerAgents = Array.isArray(metadata.owner_agents) ? metadata.owner_agents.map((entry) => normalizeString(entry)).filter(Boolean) : [];
      if (ownerAgents.length === 0) {
        skillErrors.push(`${rel(projectRoot, filePath)} não declara agent owner.`);
      }
      for (const owner of ownerAgents) {
        if (!blueprintAgentIds.has(owner)) {
          skillErrors.push(`${rel(projectRoot, filePath)} referencia owner inexistente: ${owner}.`);
        }
      }
      if (source !== 'agentic-blueprint') {
        skillErrors.push(`${rel(projectRoot, filePath)} não foi materializado pelo blueprint.`);
      }
      if (blueprintPath !== blueprintRelPath) {
        skillErrors.push(`${rel(projectRoot, filePath)} não aponta para ${blueprintRelPath}.`);
      }
    }
  }
  checks.push({
    id: 'skills_materialized',
    passed: skillErrors.length === 0,
    message: skillErrors.length === 0
      ? 'Todos os skills materializados correspondem ao blueprint.'
      : skillErrors.slice(0, 3).join(' '),
  });
  for (const message of skillErrors) {
    pushError(errors, rel(projectRoot, skillDir), message);
  }

  const contextErrors = [];
  for (const contextDoc of normalizedBlueprint.context_documents) {
    const contextPath = join(internalDir, contextDoc.path);
    if (!existsSync(contextPath)) {
      contextErrors.push(`Falta ${rel(projectRoot, contextPath)}.`);
      continue;
    }
    const content = readFileSync(contextPath, 'utf8');
    if (!content.includes(`# ${contextDoc.title}`)) {
      contextErrors.push(`${rel(projectRoot, contextPath)} não contém o título esperado.`);
    }
    if (!content.includes(`- Owner agent: \`${contextDoc.owner_agent}\``)) {
      contextErrors.push(`${rel(projectRoot, contextPath)} não declara o owner agent esperado.`);
    }
    if (!content.includes(`- Blueprint: \`${blueprintRelPath}\``)) {
      contextErrors.push(`${rel(projectRoot, contextPath)} não referencia ${blueprintRelPath}.`);
    }
  }
  for (const flow of normalizedBlueprint.flows) {
    const flowYamlPath = join(internalDir, 'flows', `${flow.id}.yaml`);
    const flowMdPath = join(internalDir, 'flows', `${flow.id}.md`);
    if (!existsSync(flowYamlPath)) {
      contextErrors.push(`Falta ${rel(projectRoot, flowYamlPath)}.`);
      continue;
    }
    if (!existsSync(flowMdPath)) {
      contextErrors.push(`Falta ${rel(projectRoot, flowMdPath)}.`);
      continue;
    }
    const flowDoc = parseYamlFile(flowYamlPath, errors, projectRoot);
    if (!flowDoc) {
      contextErrors.push(`YAML inválido em ${rel(projectRoot, flowYamlPath)}.`);
      continue;
    }
    if (normalizeString(flowDoc.generated_by) !== 'agentic-blueprint') {
      contextErrors.push(`${rel(projectRoot, flowYamlPath)} não foi materializado pelo blueprint.`);
    }
    if (normalizeBlueprintPath(flowDoc.blueprint_path) !== blueprintRelPath) {
      contextErrors.push(`${rel(projectRoot, flowYamlPath)} não aponta para ${blueprintRelPath}.`);
    }
    if (normalizeString(flowDoc.id) !== flow.id) {
      contextErrors.push(`${rel(projectRoot, flowYamlPath)} tem id diferente de ${flow.id}.`);
    }
    const flowSteps = Array.isArray(flowDoc.steps) ? flowDoc.steps : [];
    if (flowSteps.length === 0) {
      contextErrors.push(`${rel(projectRoot, flowYamlPath)} não declara steps.`);
    }
    for (const step of flowSteps) {
      const stepAgent = isPlainObject(step)
        ? normalizeString(step.agent) || normalizeString(step.agent_id) || normalizeString(step.subagent)
        : '';
      if (!stepAgent) {
        contextErrors.push(`${rel(projectRoot, flowYamlPath)} contém step sem agent.`);
        continue;
      }
      if (!blueprintAgentIds.has(stepAgent)) {
        contextErrors.push(`${rel(projectRoot, flowYamlPath)} referencia agent inexistente: ${stepAgent}.`);
      }
    }
  }
  for (const policy of normalizedBlueprint.policies) {
    const policyYamlPath = join(internalDir, 'policies', `${policy.id}.yaml`);
    const policyMdPath = join(internalDir, 'policies', `${policy.id}.md`);
    if (!existsSync(policyYamlPath)) {
      contextErrors.push(`Falta ${rel(projectRoot, policyYamlPath)}.`);
      continue;
    }
    if (!existsSync(policyMdPath)) {
      contextErrors.push(`Falta ${rel(projectRoot, policyMdPath)}.`);
      continue;
    }
    const policyDoc = parseYamlFile(policyYamlPath, errors, projectRoot);
    if (!policyDoc) {
      contextErrors.push(`YAML inválido em ${rel(projectRoot, policyYamlPath)}.`);
      continue;
    }
    if (normalizeString(policyDoc.generated_by) !== 'agentic-blueprint') {
      contextErrors.push(`${rel(projectRoot, policyYamlPath)} não foi materializado pelo blueprint.`);
    }
    if (normalizeBlueprintPath(policyDoc.blueprint_path) !== blueprintRelPath) {
      contextErrors.push(`${rel(projectRoot, policyYamlPath)} não aponta para ${blueprintRelPath}.`);
    }
    if (normalizeString(policyDoc.id) !== policy.id && normalizeString(policyDoc.name) !== policy.name) {
      contextErrors.push(`${rel(projectRoot, policyYamlPath)} não corresponde à policy esperada.`);
    }
    const policyOwners = Array.isArray(policyDoc.owner_agents) ? policyDoc.owner_agents.map((entry) => normalizeString(entry)).filter(Boolean) : [];
    if (policyOwners.length === 0) {
      contextErrors.push(`${rel(projectRoot, policyYamlPath)} não declara owner_agents.`);
    }
    for (const owner of policyOwners) {
      if (!blueprintAgentIds.has(owner)) {
        contextErrors.push(`${rel(projectRoot, policyYamlPath)} referencia owner inexistente: ${owner}.`);
      }
    }
  }
  checks.push({
    id: 'context_materialized',
    passed: contextErrors.length === 0,
    message: contextErrors.length === 0
      ? 'Contexto, flows e policies materializados correspondem ao blueprint.'
      : contextErrors.slice(0, 3).join(' '),
  });
  for (const message of contextErrors) {
    pushError(errors, rel(projectRoot, join(internalDir, 'context')), message);
  }

  const entrypointErrors = [];
  for (const entrypointPath of blueprintEntrypointPaths) {
    const absPath = join(projectRoot, entrypointPath);
    if (!existsSync(absPath)) {
      entrypointErrors.push(`Falta ${entrypointPath}.`);
      continue;
    }
    const inspection = inspectManagedEntrypointContent(readFileSync(absPath, 'utf8'));
    if (!inspection.hasBlock) {
      entrypointErrors.push(`${entrypointPath} não contém bloco gerenciado do AgentForge.`);
    }
    for (const message of buildEntrypointQualityMessage(inspection)) {
      if (message === 'Arquivo de entrada sem bloco gerenciado do AgentForge.') continue;
      if (message.startsWith('Bootloader sem referências obrigatórias')) {
        entrypointErrors.push(`${entrypointPath}: ${message}`);
      } else if (message.startsWith('Conteúdo manual excessivo fora do bloco AgentForge')) {
        entrypointErrors.push(`${entrypointPath}: ${message}`);
      } else if (message.startsWith('Entrypoint gerenciado excede o limite de')) {
        entrypointErrors.push(`${entrypointPath}: ${message}`);
      } else if (message.startsWith('Bloco AgentForge excede o limite de')) {
        entrypointErrors.push(`${entrypointPath}: ${message}`);
      } else if (message.startsWith('Linguagem Codex-only detectada')) {
        entrypointErrors.push(`${entrypointPath}: ${message}`);
      } else if (message.startsWith('Conteúdo legado Reversa detectado')) {
        entrypointErrors.push(`${entrypointPath}: ${message}`);
      }
    }
  }
  checks.push({
    id: 'entrypoints_compiled',
    passed: entrypointErrors.length === 0,
    message: entrypointErrors.length === 0
      ? 'Entrypoints materializados e compilados com bootloader válido.'
      : entrypointErrors.slice(0, 3).join(' '),
  });
  for (const message of entrypointErrors) {
    pushError(errors, 'AGENTS.md', message);
  }

  const contextIndexPath = join(internalDir, 'harness', 'context-index.yaml');
  let contextIndexOk = false;
  if (existsSync(contextIndexPath)) {
    try {
      const contextIndexDoc = YAML.parse(readFileSync(contextIndexPath, 'utf8'));
      const adoptContexts = isPlainObject(contextIndexDoc?.task_contexts?.adopt) ? contextIndexDoc.task_contexts.adopt : {};
      const adoptAgentRefs = Array.isArray(adoptContexts.agents) ? adoptContexts.agents : [];
      contextIndexOk = normalizeBlueprintPath(contextIndexDoc?.generated_by) === 'agentic-blueprint'
        && normalizeBlueprintPath(contextIndexDoc?.blueprint_path) === blueprintRelPath
        && Array.isArray(adoptAgentRefs)
        && adoptAgentRefs.includes('.agentforge/agents/')
        && Array.isArray(adoptContexts.context)
        && adoptContexts.context.includes('.agentforge/context/')
        && Array.isArray(adoptContexts.skills)
        && adoptContexts.skills.includes('.agentforge/skills/')
        && Array.isArray(adoptContexts.flows)
        && adoptContexts.flows.includes('.agentforge/flows/')
        && Array.isArray(adoptContexts.policies)
        && adoptContexts.policies.includes('.agentforge/policies/')
        && Array.isArray(adoptContexts.references)
        && adoptContexts.references.includes('.agentforge/references/');
    } catch {
      contextIndexOk = false;
    }
  }
  checks.push({
    id: 'context_index_matches_blueprint',
    passed: contextIndexOk,
    message: contextIndexOk
      ? 'context-index referencia o blueprint e as superfícies adotadas.'
      : 'context-index não reflete o blueprint validado.',
  });
  if (!contextIndexOk) {
    pushError(errors, rel(projectRoot, contextIndexPath), 'context-index não reflete o blueprint validado.');
  }

  const contextMapPath = join(internalDir, 'harness', 'context-map.yaml');
  let contextMapOk = false;
  if (existsSync(contextMapPath)) {
    try {
      const contextMapDoc = YAML.parse(readFileSync(contextMapPath, 'utf8'));
      const validation = validateContextMapFile(projectRoot, contextMapDoc);
      if (validation.errors.length === 0) {
        const currentPaths = new Set(Array.isArray(contextMapDoc?.items)
          ? contextMapDoc.items.map((item) => normalizeBlueprintPath(item.file))
          : []);
        const requiredPaths = new Set([
          ...normalizedBlueprint.context_documents.map((doc) => normalizeBlueprintPath(doc.path)),
          ...normalizedBlueprint.flows.map((flow) => `flows/${flow.id}.yaml`),
          ...normalizedBlueprint.policies.map((policy) => `policies/${policy.id}.yaml`),
        ]);
        contextMapOk = [...requiredPaths].every((path) => currentPaths.has(path));
      }
    } catch {
      contextMapOk = false;
    }
  ...
  checks.push({
    id: 'context_map_updated',
    passed: contextMapOk,
    message: contextMapOk
      ? 'context-map contém os itens gerados pela adoção.'
      : 'context-map não foi atualizado com a adoção materializada.',
  });
  if (!contextMapOk) {
    pushError(errors, rel(projectRoot, contextMapPath), 'context-map não foi atualizado com a adoção materializada.');
  }
...
function buildReport(result) {
  ...
  if (Array.isArray(result.adoptionChecks) && result.adoptionChecks.length > 0) {
    ...
    lines.push(`- Agentic adoption validation: ${passed}/${result.adoptionChecks.length}`);
  }
  lines.push('');
  ...
  if (Array.isArray(result.adoptionChecks) && result.adoptionChecks.length > 0) {
    lines.push('## Agentic adoption validation');
    lines.push('');
...
export function validateAgentForgeStructure(projectRoot) {
  ...
  const stateRef = rel(projectRoot, statePath);
  const adoptionChecks = validateAgenticAdoptionSurface(projectRoot, internalDir, state, errors);
