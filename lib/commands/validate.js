...
import { SETUP_MODES, resolveInternalDir } from '../product.js';
...
function validatePathObjectCollection({
  ...
  for (const [index, entry] of entries.entries()) {
    ...
    validateReferencedFile(baseDir, entry.path, itemRef, errors);
  }
}
...
function validateStringPathCollection({
  ...
  for (const [index, entry] of entries.entries()) {
    const itemRef = `${sourceFile}#${sectionName}[${index}]`;
    validateReferencedFile(baseDir, entry, itemRef, errors);
  }
}
...
function validateContextIndex(internalDir, projectRoot, errors) {
...
function validateTaskModes(internalDir, projectRoot, errors) {
  ...
  for (const modeName of SETUP_MODES) {
...
export function validateAgentForgeStructure(projectRoot) {
  ...
  validateContextIndex(internalDir, projectRoot, errors);
  validateTaskModes(internalDir, projectRoot, errors);
