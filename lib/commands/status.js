...
function buildStatusPayload({
  ...
  const repairApplied = repairedFields.length > 0 && unrepairedFields.length === 0;
  return {
    ...
    activation_mode: activationPlan.mode,
    current_phase: activationPlan.current_phase ?? state.workflow?.current_phase ?? state.phase ?? null,
    next_phase: activationPlan.next_phase,
    recommended_command: activationPlan.recommended_command,
    next_action: activationPlan.next_action,
    reason: activationPlan.reason,
    required_commands: activationPlan.required_commands,
    ...
    unrepaired_fields: unrepairedFields,
...
function printHumanStatus(chalk, payload) {
  ...
  console.log(`  Activation mode:     ${chalk.cyan(payload.activation_mode)}`);
  ...
  console.log(`  Recommended command: ${chalk.cyan(payload.recommended_command ?? 'none')}`);
  ...
  console.log(`  Repaired fields:      ${chalk.cyan(formatList(payload.repaired_fields))}`);
  console.log(`  Unrepaired fields:    ${chalk.cyan(formatList(payload.unrepaired_fields))}`);
  console.log(`  Recommended commands: ${chalk.cyan(formatList(payload.recommended_commands))}`);
  ...
  if (payload.repair_applied) {
    console.log(`\n  ${chalk.green('Repair applied successfully.')}`);
  } else if (payload.repaired_fields.length > 0 || payload.unrepaired_fields.length > 0) {
    console.log(`\n  ${chalk.yellow('Repair completed with outstanding issues.')}`);
  }
...
export default async function status(args = []) {
  ...
  let repairedFields = [];
  let unrepairedFields = [];
  let recommendedCommands = [];
  let nextState = state;
  let nextPlanContent = stateAndPlan.planContent;
  if (parsed.repair) {
    const repaired = repairPhaseState(projectRoot);
    repairApplied = repaired.repairedFields.length > 0 && repaired.unrepairedFields.length === 0;
    repairedFields = repaired.repairedFields;
    unrepairedFields = repaired.unrepairedFields;
    recommendedCommands = repaired.recommendedCommands;
    nextState = repaired.state;
    nextPlanContent = repaired.planContent;
  }

  const effectiveSummary = parsed.repair
    ? summarizeStatePlan({
        state: nextState,
        planContent: nextPlanContent,
      })
    : summary;
  const activationPlan = resolveAgentForgeActivationPlan(projectRoot, nextState, definition);
  ...
  if (parsed.json) {
    ...
    return parsed.repair && unrepairedFields.length > 0 ? 1 : 0;
  }
