...
function buildStatusPayload({
  ...
  return {
    ...
    pending_phases: state.pending ?? [],
    next_phase: summary.nextRecommendedPhase,
...
export default async function status(args = []) {
  ...
  let nextState = state;
  if (parsed.repair) {
    const repairedState = repairStateFromPlan(state, summary);
    const changed = JSON.stringify(repairedState.pending ?? []) !== JSON.stringify(state.pending ?? []);
    if (changed && stateAndPlan.statePath) {
      writeFileSync(stateAndPlan.statePath, `${JSON.stringify(repairedState, null, 2)}\n`, 'utf8');
      repairApplied = true;
