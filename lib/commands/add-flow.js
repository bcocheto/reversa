...
function buildFlowMarkdown(flowDoc) {
  const lines = [];
  lines.push(`# ${flowDoc.name}`);
  lines.push('');
  lines.push('## Objetivo');
  lines.push('');
  lines.push(flowDoc.description);
  lines.push('');
  lines.push('## Etapas');
  lines.push('');
  for (const [index, step] of flowDoc.steps.entries()) {
    lines.push(`${index + 1}. ${step.instruction}`);
  }
  lines.push('');
  if (Array.isArray(flowDoc.outputs_expected) && flowDoc.outputs_expected.length > 0) {
    lines.push('## Saídas esperadas');
    lines.push('');
    for (const output of flowDoc.outputs_expected) {
      lines.push(`- ${output}`);
    }
    lines.push('');
  }
...
function updateContextIndex(projectRoot, flowDoc, flowMdPath) {
  const contextIndexPath = join(projectRoot, PRODUCT.internalDir, 'harness', 'context-index.yaml');
  if (!existsSync(contextIndexPath)) return false;
  ...
  const normalizedId = normalizeString(flowDoc.id);
  const normalizedPath = `flows/${normalizedId}.md`;
  const existingIndex = flows.findIndex((entry) => normalizeString(entry?.id) === normalizedId);
  const nextEntry = {
    id: normalizedId,
    path: normalizedPath,
    purpose: normalizeString(flowDoc.description) || normalizeString(flowDoc.name) || normalizedId,
  };

  if (existingIndex === -1) flows.push(nextEntry);
  else flows[existingIndex] = { ...flows[existingIndex], ...nextEntry };

  doc.flows = flows;
  mkdirSync(dirname(contextIndexPath), { recursive: true });
  writeFileSync(contextIndexPath, `${YAML.stringify(doc).trim()}\n`, 'utf8');
  return true;
}
...
export function createProjectFlow(projectRoot, answers) {
  ...
  const flowMdPath = join(projectRoot, PRODUCT.internalDir, 'flows', `${flowDoc.id}.md`);
  writeFileSync(flowMdPath, buildFlowMarkdown(flowDoc), 'utf8');
  updateContextIndex(projectRoot, flowDoc, flowMdPath);
  ...
  updateState(projectRoot, (state) => {
    ...
    return {
      ...
      created_files: unique([
        ...
        rel(projectRoot, flowMdPath),
        rel(projectRoot, join(projectRoot, PRODUCT.internalDir, 'harness', 'context-index.yaml')),
      ]),
    };
  });
