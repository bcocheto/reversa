...
import { finalizeAdoptionWorkflow, repairPhaseState } from './phase-engine.js';
...
export function buildInstallOnboardingCopy({
  ...
  if (adoptionApplied) {
    return {
      label: '  Estrutura recomendada aplicada. A adoção agentic foi executada e os entrypoints foram compilados.',
      nextSteps: [
        '  Abra sua IA configurada.',
        '  Digite `agentforge`.',
        '  A adoção agentic já foi aplicada; revise o relatório se quiser repetir a migração.',
      ],
  ...
  return {
    label: '  Estrutura recomendada preparada. A adoção agentic ainda precisa ser aplicada com `agentforge adopt --apply`.',
    nextSteps: [
      '  Abra sua IA configurada.',
      '  Digite `agentforge`.',
      '  Execute `agentforge adopt --apply` para concluir a migração agentic.',
    ],
