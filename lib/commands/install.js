...
import { runAdoptPrepare } from './adopt.js';
...
export function buildInstallOnboardingCopy({
  ...
  const normalizedAdoptionStatus = typeof adoptionStatus === 'string' ? adoptionStatus.trim().toLowerCase() : '';
  const evidenceReady = normalizedAdoptionStatus === 'evidence_ready' || normalizedAdoptionStatus === 'ai_request_ready';
  ...
  if (evidenceReady || adoptionApplied) {
    return {
      label: '  Evidências coletadas e request de blueprint gerado.',
      nextSteps: [
        '  Abra sua IA configurada.',
        '  Leia `.agentforge/ai/requests/agentic-blueprint.md`.',
        '  Crie `.agentforge/ai/outbox/agentic-blueprint.yaml`.',
        '  Rode `agentforge adopt --apply --from-ai .agentforge/ai/outbox/agentic-blueprint.yaml`.',
      ],
      adoptionLine: adoptionPlanPath
        ? `  Adoção agentic: evidence_ready (${adoptionPlanPath})`
        : '  Adoção agentic: evidence_ready',
  ...
  return {
    ...
    blueprintLine: blueprintPaths?.requestPath
      ...
      : '  Blueprint da IA: necessário',
    deferredLine: hasDeferredEntrypoints
      ? `  Entrypoints adiados até \`agentforge adopt --apply --from-ai .agentforge/ai/outbox/agentic-blueprint.yaml\`: ${deferredEntrypoints.join(', ')}`
      : null,
  };
}
...
export default async function install(args) {
  ...
  if (!applyStructure && !adoptionMode) {
  ...
  try {
    ...
    if (adoptionMode) {
      ...
      const adoptionPreparation = runAdoptPrepare(projectRoot);
      if (!adoptionPreparation.ok) {
        spinner.stop();
        spinner.fail(chalk.red('A preparação da adoção agentic falhou.'));
        for (const error of adoptionPreparation.errors ?? []) {
          console.log(chalk.red(`  - ${error}`));
        }
        return 1;
      }

      adoptionPlanPath = adoptionPreparation.reportPath;
      blueprintArtifacts = {
        jsonPath: adoptionPreparation.evidenceArtifacts.jsonPath,
        dossierPath: adoptionPreparation.dossierPath,
        requestPath: adoptionPreparation.requestPath,
      };
      recordAgenticBlueprintState(projectRoot, blueprintArtifacts);

      const currentStatePath = join(projectRoot, PRODUCT.internalDir, 'state.json');
      if (existsSync(currentStatePath)) {
        const currentState = JSON.parse(readFileSync(currentStatePath, 'utf8'));
        updateStateAndManifest(projectRoot, {
          ...currentState,
          adoption: {
            ...(currentState.adoption ?? {}),
            status: 'evidence_ready',
            plan_report_path: adoptionPlanPath,
            apply_status: 'not-run',
            next_required_output: '.agentforge/ai/outbox/agentic-blueprint.yaml',
          },
    ...
    spinner.stop();
    if (adoptionMode) {
      spinner.succeed(chalk.hex('#ffa203')('Evidências coletadas e request de blueprint gerado.'));
      printInstallSummary({
        chalk,
        answers,
        selectedEngines,
    ...
    spinner.succeed(chalk.hex('#ffa203')(deferredEntrypoints.length > 0
