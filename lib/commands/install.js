...
export default async function install(args) {
  ...
  try {
    ...
    const { runFullCycle } = await inquirer.prompt([{
      ...
      loop: false,
      choices: [
        { name: 'Sim, executar todas as fases seguras', value: true },
        { name: 'Não, deixar para avançar depois com agentforge advance', value: false },
      ],
      default: existing.installed ? true : false,
    }]);
