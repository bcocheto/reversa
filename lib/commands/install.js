...
export default async function install(args) {
  ...
  const deferExistingEntrypoints = answers.setup_mode === 'adopt' || answers.setup_mode === 'hybrid';
  ...
  if (setupMode === 'adopt' || setupMode === 'hybrid') {
    ...
    const { finalize } = await inquirer.prompt([{
      ...
      default: shouldDefaultFinalizeAdoption(setupMode),
    }]);
    ...
    } else if (deferredEntrypoints.length > 0) {
