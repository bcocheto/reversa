...
export default async function install(args) {
  ...
  if (!applyStructure) {
    ...
    console.log(chalk.hex('#ffa203')('  Estrutura recomendada não aplicada. Reports e sugestões foram gerados em .agentforge/.'));
    return 0;
  }
  ...
  try {
    ...
    const persistedState = existsSync(statePath)
      ...
      : preview.stateSeed;
    persistAnalysis(projectRoot, persistedState, preview.analysisBundle, {
      ran: false,
      imported: 0,
      skipped: 0,
      reportPath: null,
