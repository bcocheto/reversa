...
export default async function compileCommand(args = []) {
  ...
  const takeoverEntrypoints = args.includes('--takeover-entrypoints');
  const includeExistingEntrypoints = args.includes('--include-existing-entrypoints');

  const result = await compileAgentForge(process.cwd(), {
    force,
    takeoverEntrypoints,
    includeExistingEntrypoints,
  });
  ...
  if (result.preservedSnapshots.length > 0) {
    console.log(`  Snapshots preservados: ${result.preservedSnapshots.join(', ')}`);
  }
