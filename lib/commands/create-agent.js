...
function buildAgentDoc(projectRoot, suggestion, { outputPath, suggestionPath }) {
  const limits = unique(toArray(suggestion.limits));
  const responsibilities = unique(toArray(suggestion.responsibilities));
  const reads = unique(toArray(suggestion.reads));
  const skills = unique(toArray(suggestion.skills));
  const flows = unique(toArray(suggestion.flows));
  const handoffTarget = 'reviewer';

  return {
    id: normalizeString(suggestion.id),
    name: normalizeString(suggestion.name) || normalizeString(suggestion.title),
    ...
    mission: normalizeString(suggestion.description) || normalizeString(suggestion.reason),
    reason: normalizeString(suggestion.reason),
    responsibilities,
    boundaries: limits,
    limits,
    reads,
    ...
    source_suggestion: suggestionPath ? rel(projectRoot, suggestionPath) : rel(projectRoot, join(projectRoot, SUGGESTION_DIR, `${normalizeString(suggestion.id)}.yaml`)),
