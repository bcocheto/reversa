...
function shouldWriteSuggestionFile(projectRoot, manifest, relPath) {
  const absPath = join(projectRoot, relPath);
...
function renderHelp(chalk) {
  ...
  console.log(`  Uso: npx ${PRODUCT.command} suggest-agents\n`);
  console.log('  Sugere agentes adequados ao projeto com base na análise local, padrões e contexto agentic existente.');
  console.log('  Gera `.agentforge/reports/agent-suggestions.md` e arquivos YAML em `.agentforge/suggestions/agents/`.\n');
}
...
function persistAgentSuggestions(projectRoot, result) {
  ...
  for (const suggestion of result.suggestions) {
    ...
    const decision = shouldWriteSuggestionFile(projectRoot, manifest, relPath);
...
export default async function suggestAgents(args = []) {
  ...
  const persisted = persistAgentSuggestions(projectRoot, result);
