...
export function getNextPhase(projectRoot, stateOverride = null, definitionOverride = null) {
  ...
  const focus = current && !getPhaseCompletionReport(projectRoot, current, state, definition).satisfied ? current : next;
...
export async function advancePhase(projectRoot, options = {}) {
  ...
  const snap = snapshot(snapshotPaths);
  ...
  try {
    for (const phaseId of sequence) {
      ...
      if (phase.id === 'export') {
        ...
        const compileResult = await compileAgentForge(projectRoot, { force: true });
