...
import { buildContextMapForProject, validateContextMapFile } from './context-map.js';
...
export const DEFAULT_PHASE_DEFINITION = Object.freeze({
  ...
  phases: [
    ...
    },
    {
      ...
      name: 'Agent Design',
      order: 20,
      purpose: 'Materializar a equipe agentic a partir de blueprint validado.',
      reads: ['ai/outbox/agentic-blueprint.yaml', 'agents/', 'context/project-overview.md', 'context/architecture.md'],
...
function hasManagedBlock(content) {
  ...
  return text.includes('<!-- agentforge:start -->') && text.includes('<!-- agentforge:end -->');
}
...
function collectIndexPaths(entries, paths) {
  if (!Array.isArray(entries)) return;
  for (const entry of entries) {
    if (entry && typeof entry === 'object' && !Array.isArray(entry) && typeof entry.path === 'string' && entry.path.trim()) {
      paths.add(entry.path.trim().replace(/\\/g, '/'));
    }
  }
}

function collectTaskContextPaths(taskContexts, paths) {
  if (!taskContexts || typeof taskContexts !== 'object' || Array.isArray(taskContexts)) return;

  for (const group of Object.values(taskContexts)) {
    if (Array.isArray(group)) {
      for (const entry of group) {
        if (typeof entry === 'string' && entry.trim()) {
          paths.add(entry.trim().replace(/\\/g, '/'));
        } else if (entry && typeof entry === 'object' && !Array.isArray(entry) && typeof entry.path === 'string' && entry.path.trim()) {
          paths.add(entry.path.trim().replace(/\\/g, '/'));
        }
      }
      continue;
...
function collectContextIndexPaths(doc) {
  const paths = new Set();
  if (!doc || typeof doc !== 'object' || Array.isArray(doc)) return paths;
  collectIndexPaths(doc.items, paths);
  collectIndexPaths(doc.skills, paths);
  collectIndexPaths(doc.flows, paths);
  collectTaskContextPaths(doc.task_contexts, paths);
  return paths;
}

function collectContextMapPaths(doc) {
  const paths = new Set();
  if (!doc || typeof doc !== 'object' || Array.isArray(doc)) return paths;
  if (!Array.isArray(doc.items)) return paths;
  for (const item of doc.items) {
    if (item && typeof item === 'object' && !Array.isArray(item) && typeof item.file === 'string' && item.file.trim()) {
      paths.add(item.file.trim().replace(/\\/g, '/'));
    }
  }
  return paths;
}

function collectSnapshotEvidence(projectRoot, entrypoints = []) {
  const evidence = new Set();
  const snapshotsRoot = join(projectRoot, PRODUCT.internalDir, 'imports', 'snapshots');
  if (!existsSync(snapshotsRoot) || !statSync(snapshotsRoot).isDirectory()) return evidence;

  for (const filePath of listFilesRecursive(snapshotsRoot, (fullPath, name) => name.toLowerCase().endsWith('.json'))) {
    try {
      const doc = JSON.parse(readFileSync(filePath, 'utf8'));
      if (!isPlainObject(doc)) continue;
      const sourcePath = normalizeString(doc.source_path);
      if (sourcePath) {
        evidence.add(sourcePath.replace(/\\/g, '/'));
      }
    } catch {
      continue;
    }
  ...
  for (const entrypoint of entrypoints) {
    evidence.add(entrypoint);
  }

  return evidence;
}

function normalizeAdoptionTargetPath(path) {
  return String(path ?? '').replace(/^\.agentforge\//, '').replace(/\\/g, '/');
}

function renderAdoptionVerificationReport({
  projectRoot,
  status,
  validationStatus,
  checks,
  ...
  lines.push('');
  if (status === 'verified') {
    lines.push('- Adoption is verified. Ask for a real task.');
  } else {
    lines.push('- Review the manual changes or rerun `agentforge adopt --apply --force` after fixing the blocked surface.');
  }
  lines.push('');
  return `${lines.join('\n').trimEnd()}\n`;
}

function renderContextMapWriteReport(projectRoot, contextMapBuild, validation) {
  const lines = [];
  lines.push('# AgentForge Context Map Write');
  lines.push('');
  lines.push(`- Project: ${basename(projectRoot)}`);
  lines.push(`- Generated items: ${Array.isArray(contextMapBuild?.generatedItems) ? contextMapBuild.generatedItems.length : 0}`);
  lines.push(`- Curated items preserved: ${Array.isArray(contextMapBuild?.existingItems) ? contextMapBuild.existingItems.filter((item) => normalizeString(item?.curation_status) === 'curated').length : 0}`);
  lines.push(`- Validation: ${validation.errors.length === 0 ? 'passed' : 'failed'}`);
  lines.push('');
  ...
  lines.push('## Result');
  lines.push('');
  lines.push(validation.errors.length === 0
    ? '- Context map written successfully.'
    : '- Context map written, but validation reported errors that need review.');
  lines.push('');

  return `${lines.join('\n').trimEnd()}\n`;
}

function writeContextMapVerificationArtifacts(projectRoot) {
  const contextMapBuild = buildContextMapForProject(projectRoot);
  const contextMapTarget = join(projectRoot, PRODUCT.internalDir, 'harness', 'context-map.yaml');
  const contextMapReportTarget = join(projectRoot, PRODUCT.internalDir, 'reports', 'context-map.md');
  const contextMapText = `${YAML.stringify(contextMapBuild.doc).trim()}\n`;
  writeText(contextMapTarget, contextMapText);

  const validation = validateContextMapFile(projectRoot, contextMapBuild.doc);
  const report = renderContextMapWriteReport(projectRoot, contextMapBuild, validation);
  writeText(contextMapReportTarget, report);

  saveManifest(projectRoot, {
    ...loadManifest(projectRoot),
    ...buildManifest(projectRoot, [
      rel(projectRoot, contextMapTarget),
      rel(projectRoot, contextMapReportTarget),
    ]),
  });
  ...
  return {
    ...contextMapBuild,
    validation,
    contextMapPath: rel(projectRoot, contextMapTarget),
    contextMapReportPath: rel(projectRoot, contextMapReportTarget),
  };
}
...
export function finalizeAdoptionWorkflow(projectRoot, state, {
  validationResult = null,
  verificationResult = null,
  adoptionApplyPath = null,
} = {}) {
  if (validationResult && validationResult.valid === false && !verificationResult) {
  ...
  const verificationStatus = normalizeString(verificationResult?.status) || (validationResult?.valid === false ? 'failed' : 'verified');
  const finalizedState = {
    ...
    adoption: {
      ...
      ...(verificationStatus === 'verified'
        ...
        : { verification_failed_at: verifiedAt }),
      ...(adoptionApplyPath ? { apply_report_path: adoptionApplyPath } : {}),
      ...(verificationResult?.reportPath ? { verification_report_path: verificationResult.reportPath } : {}),
    },
...
export function verifyAdoptionWorkflow(projectRoot, state, {
  planResult = null,
  applyResult = null,
  validationResult = null,
  adoptionApplyPath = null,
  ...
  const checks = [];
  const manifest = loadManifest(projectRoot);
  const currentState = state ?? readState(projectRoot);
  const currentValidation = validationResult ?? { valid: false, errors: [{ message: 'Validation result is missing.' }] };

  const contextMapArtifacts = writeContextMapVerificationArtifacts(projectRoot);
  const contextIndexPath = join(projectRoot, PRODUCT.internalDir, 'harness', 'context-index.yaml');
  const contextIndexDoc = readYamlObject(contextIndexPath);
  const contextMapPath = join(projectRoot, PRODUCT.internalDir, 'harness', 'context-map.yaml');
  const contextMapDoc = readYamlObject(contextMapPath);

  const agentsContent = readText(join(projectRoot, 'AGENTS.md'));
  const agentsManaged = hasManagedBlock(agentsContent);
  checks.push({
    name: 'AGENTS.md has a managed AgentForge block',
    passed: agentsManaged,
    details: agentsManaged ? 'managed block present' : 'managed block missing',
  });

  const claudePath = join(projectRoot, 'CLAUDE.md');
  if (existsSync(claudePath)) {
    const claudeManaged = hasManagedBlock(readText(claudePath));
    checks.push({
      name: 'CLAUDE.md has a managed AgentForge block',
      passed: claudeManaged,
      details: claudeManaged ? 'managed block present' : 'managed block missing',
    });
  } else {
    checks.push({
      name: 'CLAUDE.md managed block check',
      passed: true,
      details: 'CLAUDE.md does not exist',
    });
  }

  const snapshotPaths = new Set(Array.isArray(applyResult?.snapshotPaths)
    ? applyResult.snapshotPaths.map((entry) => String(entry).replace(/\\/g, '/'))
    : []);
  const entrypoints = Array.isArray(applyResult?.entrypoints) ? applyResult.entrypoints.map((entry) => String(entry).replace(/\\/g, '/')) : [];
  for (const sourcePath of collectSnapshotEvidence(projectRoot, entrypoints)) {
    snapshotPaths.add(sourcePath.replace(/\\/g, '/'));
  }
  const entrypointCount = entrypoints.length;
  const snapshotsOk = entrypointCount === 0 || entrypoints.every((entrypoint) => snapshotPaths.has(entrypoint));
  checks.push({
    name: 'Legacy entrypoint snapshots exist',
    passed: snapshotsOk,
    details: snapshotsOk ? `snapshots preserved: ${snapshotPaths.size}` : 'missing snapshot evidence for taken-over entrypoints',
  });

  const legacySkillItems = Array.isArray(planResult?.classified)
    ? planResult.classified.filter((item) => item.path.startsWith('.agents/skills/') && /\/SKILL\.md$/i.test(item.path))
    : [];
  const legacySkillDetails = [];
  let legacySkillsOk = true;
  let migrableSkillDestinationsOk = true;
  for (const item of legacySkillItems) {
    const action = normalizeString(item.action);
    const skillId = basename(dirname(item.path));
    const canonicalTarget = `.agentforge/skills/${skillId}/SKILL.md`;
    const canonicalRel = normalizeAdoptionTargetPath(canonicalTarget);
    const canonicalAbs = join(projectRoot, canonicalRel);
    const skipped = Array.isArray(applyResult?.skipped) ? applyResult.skipped.find((entry) => entry.source === item.path) : null;
    const migrated = Array.isArray(applyResult?.migrated) ? applyResult.migrated.map((entry) => normalizeAdoptionTargetPath(entry)).includes(canonicalRel) : false;

    if (action !== 'migrate') {
      legacySkillDetails.push(`${item.path} → preserved/needs-review (${action || 'not migratable'})`);
      continue;
    }

    if (migrated || existsSync(canonicalAbs)) {
      legacySkillDetails.push(`${item.path} → migrated to ${canonicalTarget}`);
      continue;
    }

    if (skipped) {
      legacySkillDetails.push(`${item.path} → preserved/needs-review (${skipped.reason})`);
      continue;
    }

    legacySkillsOk = false;
    migrableSkillDestinationsOk = false;
    legacySkillDetails.push(`${item.path} → missing canonical destination ${canonicalTarget}`);
  }

  checks.push({
    name: 'Legacy skills were migrated or marked preserved/needs-review',
    passed: legacySkillsOk,
    details: legacySkillDetails.length > 0 ? legacySkillDetails.join('; ') : 'no legacy skills found',
  });

  checks.push({
    name: 'No migrable legacy skill lacks a canonical destination',
    passed: migrableSkillDestinationsOk,
    details: migrableSkillDestinationsOk ? 'all migrable skills have a canonical target or were preserved' : 'one or more legacy skills remained without a canonical target',
  });

  const expectedIndexPaths = new Set();
  for (const migratedPath of Array.isArray(applyResult?.migrated) ? applyResult.migrated : []) {
    const normalized = normalizeAdoptionTargetPath(migratedPath);
    if (normalized.startsWith('skills/') || normalized.startsWith('flows/') || normalized.startsWith('context/')) {
      expectedIndexPaths.add(normalized);
    }
  }

  const contextIndexPaths = collectContextIndexPaths(contextIndexDoc);
  const contextIndexOk = [...expectedIndexPaths].every((path) => contextIndexPaths.has(path));
  checks.push({
    name: 'Context index lists migrated skills, flows, and context',
    passed: contextIndexOk,
    details: contextIndexOk
      ? `${expectedIndexPaths.size} migrated path(s) found in context-index.yaml`
      : `missing some migrated paths in context-index.yaml`,
  });

  const generatedContextMapPaths = collectContextMapPaths(contextMapArtifacts.doc);
  const contextMapPaths = collectContextMapPaths(contextMapDoc);
  const contextMapOk = contextMapArtifacts.validation.errors.length === 0 && [...generatedContextMapPaths].every((path) => contextMapPaths.has(path));
  checks.push({
    name: 'Context map was updated after migration',
    passed: contextMapOk,
    details: contextMapOk
      ? `${generatedContextMapPaths.size} generated path(s) validated in context-map.yaml`
      : 'context-map.yaml does not match the generated workspace map',
  });

  const validationOk = currentValidation.valid !== false;
  checks.push({
    name: 'Validation passes',
    passed: validationOk,
    details: validationOk ? 'validate succeeded' : 'validate failed',
  });

  const verificationStatus = checks.every((check) => check.passed) ? 'verified' : 'failed';
  const recommendedCommands = verificationStatus === 'verified'
    ? []
    : ['agentforge adopt --apply --force', 'agentforge validate'];
  const reportPath = join(projectRoot, PRODUCT.internalDir, 'reports', 'adoption-verification.md');
  const report = renderAdoptionVerificationReport({
    projectRoot,
    status: verificationStatus,
    validationStatus: validationOk ? 'passed' : 'failed',
    checks,
    recommendedCommands,
    contextIndexPath: existsSync(contextIndexPath) ? rel(projectRoot, contextIndexPath) : null,
    contextMapPath: existsSync(contextMapPath) ? rel(projectRoot, contextMapPath) : null,
    contextMapReportPath: existsSync(join(projectRoot, PRODUCT.internalDir, 'reports', 'context-map.md'))
      ? rel(projectRoot, join(projectRoot, PRODUCT.internalDir, 'reports', 'context-map.md'))
      : null,
    adoptionApplyPath,
  });
  ...
  saveManifest(projectRoot, {
    ...manifest,
    ...buildManifest(projectRoot, [rel(projectRoot, reportPath)]),
  });
