...
export function getNextPhase(projectRoot, stateOverride = null, definitionOverride = null) {
  ...
  return {
    ...
    commands: ['agentforge advance', next ? `agentforge advance --phase ${next.id}` : null, 'agentforge advance --all'].filter(Boolean),
  };
}
...
export function renderPlanFromPhases(projectRoot, stateOverride = null, definitionOverride = null) {
  ...
  lines.push('> Do not edit checkboxes manually. Use `agentforge advance` or `agentforge status --repair`.');
  lines.push('');
...
export async function advancePhase(projectRoot, options = {}) {
  ...
  const compileOptions = {
    force: true,
    takeoverEntrypoints: false,
    includeExistingEntrypoints: false,
  ...
  if (targetId && targetIndex === -1) {
    return { ok: false, errors: [`Unknown phase: ${targetId}`], results: [] };
  }
  if (!options.all && targetId && targetIndex < currentIndex) {
    return { ok: false, errors: [`Target phase ${targetId} is behind the current phase ${workflow.current_phase}.`], results: [] };
  }

  const sequence = phaseIds.slice(currentIndex, endIndex + 1);
  const snapshotPaths = new Set([stateFile(projectRoot), planFile(projectRoot), historyFile(projectRoot), ...phaseFiles(definition).map((relPath) => join(projectRoot, relPath))]);
  for (const target of ENTRYPOINT_TARGETS) {
    snapshotPaths.add(join(projectRoot, target.path));
  }
  let workingState = normalizeStateForWrite(state, definition);
  const results = [];
  let snap = null;

  try {
    snap = snapshot(snapshotPaths);
    for (const phaseId of sequence) {
      const phase = definition.phases.find((entry) => entry.id === phaseId);
      if (!phase) continue;

      const checks = phaseCompletionMessages(projectRoot, phase, workingState, definition);
      let written = writePhaseArtifacts(projectRoot, workingState, phase);

      if (phase.id === 'export') {
        const { compileAgentForge } = await import('../exporter/index.js');
        const compileResult = await compileAgentForge(projectRoot, compileOptions);
        if (compileResult.errors?.length > 0) {
          throw new Error(compileResult.errors[0] ?? 'Phase export compile failed.');
        }
        written = [...written, ...(compileResult.written ?? [])];
        workingState = normalizeStateForWrite(mergeExportedState(projectRoot, workingState), definition);
      }

      const completed = normalizePhaseList(workingState.workflow.completed_phases);
      if (!completed.includes(phase.id)) completed.push(phase.id);
      const next = nextPhase(definition, phase.id)?.id ?? phase.id;
      const pending = phaseIds.filter((id) => !completed.includes(id));
      workingState = {
        ...workingState,
        workflow: { current_phase: next, completed_phases: completed, pending_phases: pending, phase_history: [...workingState.workflow.phase_history] },
        phase: next,
        completed,
        pending,
      };

      writeText(stateFile(projectRoot), `${JSON.stringify(workingState, null, 2)}\n`);
      writeText(planFile(projectRoot), renderPlanFromPhases(projectRoot, workingState, definition));
      const validation = await runValidate(projectRoot);
      if (validation.errors?.length > 0) {
        throw new Error(validation.errors[0]?.message ?? 'Phase validation failed.');
      }

      const history = buildHistoryRecord({
        from: phase.id,
        to: next,
        command: options.all ? 'advance --all' : (targetId ? `advance --phase ${targetId}` : 'advance'),
        written,
        checks,
        validation: validation.valid ? 'passed' : 'failed',
      });
      appendHistory(projectRoot, history);
      workingState.workflow.phase_history.push(history);
      writeText(stateFile(projectRoot), `${JSON.stringify(workingState, null, 2)}\n`);

      results.push({ phase: phase.id, next, written, checks, validation: validation.valid ? 'passed' : 'failed' });
    }

    syncPhaseManifest(projectRoot, results.flatMap((result) => result.written ?? []));
  } catch (error) {
    if (snap) {
      restoreSnapshot(snap);
    }
    return { ok: false, errors: [error instanceof Error ? error.message : String(error)], results };
  }
  ...
  return {
    ...
    results,
    state: workingState,
    workflowComplete: isWorkflowComplete(workingState.workflow, definition),
...
export function renderAdvanceReport(result) {
  ...
  if (!result?.ok) {
    ...
    lines.push('');
    lines.push('- Fix the blocking issue and run `agentforge advance --all` again.');
  ...
  for (const step of result.results ?? []) {
    lines.push(`- ${step.phase} -> ${step.next}`);
  }
  lines.push('');

  lines.push('## Files written');
  lines.push('');
  const written = new Set((result.results ?? []).flatMap((step) => step.written ?? []));
  if (written.size === 0) {
    lines.push('- none');
  } else {
  ...
  lines.push('');
  const decisions = (result.results ?? []).flatMap((step) => step.checks ?? []);
  ...
  lines.push('');
  for (const step of result.results ?? []) {
    lines.push(`- ${step.phase}: ${step.validation}`);
  }
  lines.push('');
  ...
  if (result.workflowComplete) {
    lines.push('- Open Codex and start a real task with `agentforge`.');
    lines.push('- Run `agentforge validate` after any manual change in `.agentforge/`.');
  } else {
    lines.push('- Continue with the next phase using `agentforge advance`.');
  }
  lines.push('');
...
export function repairPhaseState(projectRoot) {
  const definition = loadPhaseDefinition(projectRoot);
  const state = normalizeStateForWrite(readState(projectRoot), definition);
  writeText(stateFile(projectRoot), `${JSON.stringify(state, null, 2)}\n`);
  const plan = renderPlanFromPhases(projectRoot, state, definition);
  writeText(planFile(projectRoot), plan);
  return { state, planContent: plan };
}
