...
function resolvePackPaths(index, modeInfo) {
  ...
  const resolved = {
    ...
    blueprint: unique(toArray(taskSpec.blueprint).map(normalizeInternalPath)),
...
function renderMarkdown(pack) {
  ...
  const sectionItems = (section) => pack.entries.filter((item) => {
    if (!pack.generic) {
      ...
      if (section === 'blueprint') return pack.blueprint.includes(item.path);
  ...
  if (!pack.generic) {
    ...
    renderList('Blueprint', sectionItems('blueprint'));
    if (sectionItems('suggestions').length > 0) {
