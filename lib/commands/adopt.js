  ...
  return {
    ...
    task_contexts: {
      ...
      'agent-design': {
        ...
        blueprint: [
          'ai/outbox/agentic-blueprint.yaml',
        ],
        memory: [
          'memory/',
        ],
...
function renderAdoptionPlan({
  ...
  lines.push('- `agentforge adopt --apply --from-ai <path>`');
  ...
  lines.push('- Canonical AgentForge surfaces must be changed through `agentforge adopt --apply --from-ai <path>`, not by manual YAML edits.');
...
export default async function adopt(args = []) {
  ...
  if (help) {
    ...
    console.log(`  Uso: npx ${PRODUCT.command} adopt [--prepare] | adopt --apply --from-ai <path>\n`);
    ...
    console.log('  Com --apply, o comando exige --from-ai <path> e materializa a arquitetura agentic validada pela IA.');
    console.log('  Sem --from-ai, o comando falha com erro explícito e não usa heurísticas.');
    console.log('  Com --from-ai, o comando lê um blueprint da IA e materializa a arquitetura agentic validada.');
  ...
  if (apply) {
    if (fromAiIndex < 0) {
      console.log(chalk.red('  Missing agentic blueprint. Run `agentforge adopt --prepare`, let the active AI generate `.agentforge/ai/outbox/agentic-blueprint.yaml`, then run `agentforge adopt --apply --from-ai <path>`.\n'));
      return 1;
    }
