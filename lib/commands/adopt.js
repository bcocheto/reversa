...
function updateAdoptionState(projectRoot, status) {
...
export async function runAdoptApply(projectRoot) {
  ...
  if (!ingestResult.ok) {
    ...
    return { ok: false, errors, step: 'ingest' };
  }

  const auditResult = runContextAudit(projectRoot);
  if (!auditResult.ok) {
    errors.push(auditResult.errors[0]);
    return { ok: false, errors, step: 'audit-context' };
  }
  writeContextAudit(projectRoot, auditResult.analysis, auditResult.report);

  const refactorResult = await runRefactorContext(projectRoot, { apply: true });
  if (!refactorResult.ok) {
    errors.push(refactorResult.errors[0]);
    return { ok: false, errors, step: 'refactor-context' };
  }
  const refactorApplyResult = await applyRefactorContext(projectRoot, refactorResult);

  const contextResult = writeCoreContextFiles(projectRoot);

  const skillsResult = runSkillSuggestions(projectRoot);
  if (!skillsResult.ok) {
    errors.push(skillsResult.errors[0]);
    return { ok: false, errors, step: 'suggest-skills' };
  }

  const compileResult = await compileAgentForge(projectRoot, { takeoverEntrypoints: true });
  if (compileResult.errors.length > 0) {
    errors.push(compileResult.errors[0]);
    return { ok: false, errors, step: 'compile' };
  }
  ...
  if (!validationResult.valid) {
    errors.push(validationResult.errors[0]?.message ?? 'Validation failed.');
    return { ok: false, errors, step: 'validate' };
  }

  updateAdoptionState(projectRoot, 'applied');
