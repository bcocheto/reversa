...
export default async function addEngine(args) {
  ...
  if (!Array.isArray(state.internal_agents ?? state.agents) || (state.internal_agents ?? state.agents).length === 0) {
    console.log(chalk.red('  state.json has no internal agents.'));
    console.log('  Run ' + chalk.bold(`npx ${LEGACY_PRODUCT.command} install`) + ' first.\n');
    return;
  }

  const installedEngineIds = new Set(state.engines ?? []);
  const installedAgents = state.internal_agents ?? state.agents;
