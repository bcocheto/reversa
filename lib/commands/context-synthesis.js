...
function safeString(value, fallback = 'Não detectado') {
...
function buildProjectOverviewDocument(bundle, state, planSummary) {
  ...
  const projectType = safeString(signals.projectType || state?.project_type, 'Não detectado');
  const objective = safeString(signals.objectiveText || state?.objective, 'Não detectado');
  const audience = safeString(signals.audienceText, 'Não detectado');
  const stack = analysis?.detectedStack?.length > 0 ? analysis.detectedStack.join(', ') : safeString(analysis?.framework, 'Não detectado');
  ...
  const lines = [
    ...
    `- Próxima fase recomendada: ${planSummary.nextRecommendedPhase ?? 'a confirmar'}.`,
    '',
    '## A confirmar',
    '',
  ];
...
function buildGlossaryDocument(analysis) {
  ...
  push('Next phase', 'Próxima fase recomendada com base no state e no plan.');
...
function buildReadmeDocument(bundle, state, planSummary) {
  ...
  const projectName = safeString(signals.projectName || state?.project || state?.project_name, 'Projeto não detectado');
  const projectType = safeString(signals.projectType || state?.project_type, 'Não detectado');
  const nextPhase = planSummary.nextRecommendedPhase ?? 'a confirmar';
  ...
  const lines = [
    ...
    `- Projeto: ${projectName}`,
    `- Tipo: ${projectType}`,
    `- Objetivo: ${safeString(signals.objectiveText || state?.objective, 'Não detectado')}`,
    `- Stack: ${analysis?.detectedStack?.length > 0 ? analysis.detectedStack.join(', ') : safeString(analysis?.framework, 'Não detectado')}`,
    `- Públicos/usuários: ${safeString(signals.audienceText, 'Não detectado')}`,
    '',
    '## Instalação',
    '',
    `- Modo de instalação: ${setupMode}`,
    `- Engines ativas: ${engines.length > 0 ? engines.join(', ') : 'Não detectado'}`,
  ...
  if (planSummary.nextRecommendedPhase === null) missing.push('Próxima fase recomendada a partir do plan.md.');
