...
function classifySnippet(snippet, source) {
  ...
  if (best.target.path === '.agentforge/context/testing.md' && !/\btest(s|ing)?\b/i.test(title + ' ' + body) && commandLike) {
    return {
...
function renderContextIndexDoc(analysis, writtenPaths, existingContextIndex) {
  const currentPaths = new Set(writtenPaths.map((path) => path.replace(/^\.agentforge\//, '')));
  ...
  const items = TARGET_DEFINITIONS
    .filter((target) => target.path !== '.agentforge/context/unclassified.md')
    .filter((target) => currentPaths.has(target.path))
    .map((target) => ({
      id: target.path.replace('.agentforge/', '').replace(/\.md$/, '').replace(/\//g, '-'),
      path: target.path.replace('.agentforge/', ''),
...
async function applyRefactorContext(projectRoot, result, { force = false } = {}) {
  const manifest = loadManifest(projectRoot);
