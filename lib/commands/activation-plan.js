...
export function resolveAgentForgeActivationPlan(projectRoot, state, definition, options = {}) {
  ...
  if (adoption.adoption_status === 'planned' || adoption.apply_status === 'pending') {
    return {
      ...
      next_phase: null,
      recommended_command: 'agentforge adopt --prepare',
      next_action: 'prepare-adoption-evidence',
      reason: 'A adoção está planejada ou a aplicação ainda está pendente; gere evidências antes de aplicar o blueprint.',
      required_commands: ['agentforge adopt --prepare'],
      phase_engine: null,
    };
  }

  if (adoption.adoption_status === 'evidence_ready' || adoption.adoption_status === 'ai_request_ready') {
    const blueprintPath = adoption.next_required_output || '.agentforge/ai/outbox/agentic-blueprint.yaml';
    return {
      mode: adoption.adoption_status === 'ai_request_ready' ? 'adoption-ai-request-ready' : 'adoption-evidence-ready',
      should_continue_workflow: false,
      current_phase: adoption.adoption_status === 'ai_request_ready' ? 'adoption-ai-request-ready' : 'adoption-evidence-ready',
      next_phase: null,
      ...
      required_commands: [
        blueprintPath,
        `agentforge adopt --apply --from-ai ${blueprintPath}`,
      ],
      phase_engine: null,
    };
  }
  ...
  if (adoption.adoption_status === 'applied' && adoption.verification_status === 'failed') {
    ...
    return {
      ...
      next_phase: null,
      recommended_command: `agentforge adopt --apply --from-ai ${blueprintPath} --force`,
      ...
      required_commands: [
        `agentforge adopt --apply --from-ai ${blueprintPath} --force`,
        'agentforge validate',
      ],
      phase_engine: null,
    };
