...
function isAdoptionHandoff({ mode, task, state } = {}) {
  ...
  return (
    ...
    || adoptionStatus === 'evidence_ready'
    || adoptionStatus === 'ai_request_ready'
...
export function buildHandoffData(projectRoot, { engine = '', phase = '', mode = '', task = '' } = {}) {
  ...
  const isAdoptionComplete = activationPlan.mode === 'adoption-complete';
  const currentPhaseId = isAdoptionComplete ? null : (activationPlan.current_phase ?? null);
  ...
  const commands = continueWorkflow
    ...
    : (activationPlan.mode?.startsWith('adoption-')
      ? activationPlan.required_commands
  ...
  if ((focusPhase?.id ?? currentPhaseId) === 'agent-design') {
    commands.splice(1, 0,
      ...
      'npx @bcocheto/agentforge apply-suggestions --blueprint .agentforge/ai/outbox/agentic-blueprint.yaml',
    );
  }
  ...
  return {
    ...
    next_phase: isAdoptionComplete ? null : (continueWorkflow ? (focusNextPhase?.id ?? phaseEngine?.nextPhase?.id ?? null) : null),
    workflow_complete: isAdoptionComplete ? true : (continueWorkflow ? Boolean(phaseEngine?.workflowComplete) : false),
    instructions,
    ...
    next_step: isAdoptionComplete
      ? 'ask-for-real-task'
      : (continueWorkflow
        ? (focusNextPhase?.id ?? phaseEngine?.nextPhase?.id ?? 'none')
        : (activationPlan.next_action ?? activationPlan.required_commands[0] ?? 'none')),
...
export function renderHandoffReport(data) {
  ...
  lines.push(`- Próxima fase: ${data.activation_plan?.mode === 'adoption-complete' ? 'nenhuma fase estrutural pendente' : (data.current_phase ?? 'discovery')}`);
  ...
  if (data.activation_plan?.mode === 'adoption-complete') {
    lines.push('- A adoção agentic já foi aplicada e verificada.');
    lines.push('- Não execute discovery/agent-design automaticamente.');
    lines.push('- Aguarde a próxima tarefa real do projeto.');
  }
  if (data.activation_plan?.mode === 'adoption-evidence-ready' || data.activation_plan?.mode === 'adoption-ai-request-ready') {
    lines.push('- Não execute review.');
    lines.push('- Não avance phase-engine.');
    lines.push('- A tarefa atual é gerar o blueprint da IA.');
    lines.push('- Não materialize agentes manualmente.');
  }
  ...
  if (data.activation_plan?.mode === 'adoption-complete') {
    lines.push('- Não há fase estrutural pendente.');
    lines.push('- Não execute discovery/agent-design automaticamente.');
    lines.push('- Aguarde a próxima tarefa real do projeto.');
  } else if (data.activation_plan?.mode === 'adoption-evidence-ready' || data.activation_plan?.mode === 'adoption-ai-request-ready') {
    lines.push('- Não execute review.');
    lines.push('- Não avance phase-engine.');
    lines.push('- A tarefa atual é gerar o blueprint da IA.');
    lines.push('- Não materialize agentes manualmente.');
  ...
  if (data.write_policy) {
    ...
    if (data.write_policy.adoption) {
      lines.push('- Adoção/hybrid detectada: superfícies canônicas devem mudar via `agentforge adopt --apply --from-ai .agentforge/ai/outbox/agentic-blueprint.yaml`, não manualmente.');
    }
    ...
    if ((data.write_policy.phase ?? '') === 'agent-design') {
      lines.push('- Nunca crie ou edite agentes manualmente.');
      lines.push('- Agentes finais vêm de blueprint validado ou de `agentforge create-agent <id>` / `agentforge apply-suggestions --blueprint .agentforge/ai/outbox/agentic-blueprint.yaml`.');
      lines.push('- Se não houver blueprint validado, pare e peça `.agentforge/ai/outbox/agentic-blueprint.yaml`.');
      lines.push('- Não compare sugestões heurísticas para decidir criação de agentes.');
    }
    ...
    if (data.write_policy.adoption) {
      lines.push('- Superfícies canônicas do adopt só devem mudar por `agentforge adopt --apply --from-ai .agentforge/ai/outbox/agentic-blueprint.yaml`.');
    }
    lines.push('');
  }
...
export default async function handoff(args = []) {
  ...
  if (options.json) {
    ...
    if (data.activation_plan?.mode === 'adoption-complete') {
      console.log('Próxima fase: nenhuma fase estrutural pendente');
      console.log('Aguarde a próxima tarefa real do projeto.');
    } else {
