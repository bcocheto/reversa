...
export default async function update(args) {
  ...
  if (!semver.valid(installedVersion)) {
    console.log(chalk.yellow(`  Invalid installed version: "${installedVersion}". Run npx ${LEGACY_PRODUCT.command} install to fix it.\n`));
    return;
  }
  ...
  const latestVersion =
    await fetchLatestVersion(PRODUCT.packageName) ??
    await fetchLatestVersion(LEGACY_PRODUCT.packageName);
  spinner.stop();
