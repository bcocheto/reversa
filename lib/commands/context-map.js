...
function isRangeValid(projectRoot, item) {
  const relPath = normalizeFilePath(String(item?.file ?? ''));
  const absPath = join(projectRoot, PRODUCT.internalDir, relPath);
  if (!relPath || !existsSync(absPath) || statSync(absPath).isDirectory()) return false;
  const startLine = item?.start_line;
  const endLine = item?.end_line;
  if (!Number.isInteger(startLine) || !Number.isInteger(endLine) || startLine < 1 || endLine < 1 || startLine > endLine) {
    return false;
  }
  const lineCount = countLines(readText(absPath));
  return lineCount > 0 && startLine <= lineCount && endLine <= lineCount;
}
...
function mergeContextMapItems(projectRoot, existingItems = [], generatedItems = []) {
  ...
  for (const item of existingItems) {
    ...
    if (normalizeString(item.curation_status) === 'curated') {
      ...
      if (!isRangeValid(projectRoot, preserved)) {
        preserved.curation_status = 'stale';
      }
      generatedById.set(id, preserved);
    }
  }
...
function buildContextMapDoc(projectRoot) {
  ...
  const items = mergeContextMapItems(projectRoot, existingItems, generated);
